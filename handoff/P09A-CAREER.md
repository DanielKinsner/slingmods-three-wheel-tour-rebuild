...
- Integrated full loop passed in `director-kit/production/evidence/P09A/career-loop-07/`, including completed Cup-leg held-R return to the hub. The earlier complete loop is retained in `career-loop-05/`. Earlier runs and failures are retained: missing root start/reset permit, HMR interruption, and a controlled-clock harness that needed one final render after asynchronous receipt commit. Final runtime reruns supersede this candidate evidence.
