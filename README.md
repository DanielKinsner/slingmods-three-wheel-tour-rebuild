...
P05 evidence, full-field traces, source hashes, reviewer notes and native-RAF measurements live under `director-kit/production/evidence/P05/`. `build-inputs-verified.json` identifies the captured runtime at 7fe1e88. Read `performance-review.md` for the mixed original matrix, followups and unresolved findings; performance acceptance remains held. `scripts/build-review10.mjs` builds and freezes new input hashes; the original `scripts/run-review10-matrix.ps1` names preserved evidence directories and must not be rerun over them. Use fresh names as described in `AUTONOMOUS_RESUME.md`. Controlled-clock rule/UI tests are separate from real-time performance and continuous wall-clock video.
