...
try{
 ...
 await action('quick-race');await page.locator('[data-action="race"][data-value="express"]').first().click();await page.waitForFunction(()=>!!window.__SIGNATURE?.inspect().departure);
 ...
 let s=await page.evaluate(()=>window.__EXPRESS.inspect());assert.equal(s.vehicleContext.year,2026);assert.equal(Object.keys(s.rivals).length,3);assert.ok(s.rivals.maya.optics.roles.includes('brake'));
 const rivalImage=await page.evaluate(()=>window.__EXPRESS.referenceCamera('maya',[2.8,1.1,-3.2],[0,.65,-.25]));await fs.writeFile(out+'/rival.png',Buffer.from(rivalImage.split(',')[1],'base64'));report.rival=s.rivals.maya;assert.deepEqual(s.recipe,recipe);assert.match(s.visual.asset,/model02/);assert.equal(s.handlingProfile,'slingmods-sport-v4');
 ...
 const oldRecipe={...recipe,handlingProfile:'slingmods-sport-v1'};await page.goto(base+'/?scene=signature&screen=build&test=1&profile=1#build='+encodeURIComponent(JSON.stringify(oldRecipe)));await ready('__SIGNATURE');assert.deepEqual((await state()).recipe,oldRecipe);assert.equal((await state()).vehicleContext.year,2026);report.oldRecipeLoaded=true;
