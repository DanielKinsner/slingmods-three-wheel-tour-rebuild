"""Verify the retrieved Review19 ZIP and every nested data member without extracting."""
...
receipt = json.loads((root/'handoff/P09C-PACKET.json').read_text(encoding='utf-8'))
archive = root/'Astra-Review-19-Lean.zip'
...
with zipfile.ZipFile(archive) as package:
    ...
    seen = set()
    with tarfile.open(fileobj=io.BytesIO(package.read('verification/P09C-complete-data.tar.xz')), mode='r:xz') as data_archive:
        for member in data_archive:
            ...
            assert len(data) == expected['memberBytes'] and hashlib.sha256(data).hexdigest() == expected['memberSHA256'], member.name
