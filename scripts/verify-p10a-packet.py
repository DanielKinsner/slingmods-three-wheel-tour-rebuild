"""Verify the retrieved Review20 ZIP and every nested data member without extracting."""
...
receipt = json.loads((root/'handoff/P10A-PACKET.json').read_text(encoding='utf-8'))
archive = root/'Astra-Review-20-Lean.zip'
...
with zipfile.ZipFile(archive) as package:
    ...
    seen = set()
    verified_payloads = {}
    with tarfile.open(fileobj=io.BytesIO(package.read('verification/P10A-complete-data.tar.xz')), mode='r:xz') as data_archive:
