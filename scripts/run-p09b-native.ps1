...
$env:RACES='2'
Remove-Item Env:P09B_TRACE -ErrorAction SilentlyContinue
...
foreach ($width in @(1280,1920)) {
 foreach ($route in @('harbor','express')) {
  foreach ($equipment in @('stock','equipped')) {
   ...
   $env:ROUTE=$route
   $env:BUILD=$equipment
   $env:EVIDENCE_DIR="director-kit/production/evidence/P09B/$caseName"
   Write-Output "BEGIN $caseName"
   node scripts/profile-p09b.mjs *> ".p09b-$caseName.log"
