...
const base=process.env.BASE_URL||'http://127.0.0.1:5205';const out='.tools/model02-smoke';await fs.mkdir(out,{recursive:true});
...
try{
 ...
 assert.ok(Math.abs((await state()).display.mount.width-.155)<1e-6);
 ...
 await page.evaluate(()=>localStorage.setItem('model02-career-sentinel','unchanged'));
 report.finishes=[];
 ...
 await action('save-menu');await page.locator('#signature-recipe-name').fill('2026 integration smoke');await action('save');
 await page.reload();await ready('__SIGNATURE');assert.deepEqual((await state()).recipe,recipe);assert.ok((await state()).saved.some(r=>r.name==='2026 integration smoke'));
 ...
 let s=await page.evaluate(()=>window.__EXPRESS.inspect());assert.deepEqual(s.recipe,recipe);assert.match(s.visual.asset,/model02/);assert.equal(s.handlingProfile,'slingmods-sport-v4');
 ...
 await page.locator('#race-menu [data-action=bay]').click();await ready('__SIGNATURE');assert.deepEqual((await state()).recipe,recipe);assert.match((await state()).visual.asset,/model02/);assert.equal(await page.evaluate(()=>localStorage.getItem('model02-career-sentinel')),'unchanged');
