...
try{
 ...
 await page.keyboard.down('KeyR');await page.waitForTimeout(600);await page.keyboard.press('Escape');await page.waitForTimeout(600);await page.keyboard.press('Escape');await page.waitForTimeout(600);const interruptedHold=await page.evaluate(()=>window.__TWT.telemetry());assert.ok(interruptedHold.time>5);await page.keyboard.up('KeyR');await page.keyboard.down('KeyR');await page.waitForFunction(()=>{const t=window.__TWT.telemetry();return t.time<.5&&Math.abs(t.position.z-35)<.1},null,{timeout:4000});await page.keyboard.up('KeyR');const reset=await page.evaluate(()=>window.__TWT.telemetry());assert.ok(Math.abs(reset.position.x)<.1&&Math.abs(reset.position.z-35)<.1);
