...
def add(p,name=None):
 p=Path(p);assert p.is_file(),p;assert not any(x in p.parts for x in ['.git','node_modules','dist','__pycache__','.tools','raw-video','agent','solo-agent','crew-agent','fixture']);assert not p.name.startswith('.env') and not re.search(r'\.blend\d+$',p.name) and p.suffix not in ['.bak','.key','.pem','.zip','.pyc'];files[name or p.as_posix()]=p
...
for n,h in build['inputs'].items():assert hashlib.sha256(Path(n).read_bytes()).hexdigest()==h,n;add(n)
for folder in ['public','assets/blender/showcase']:tree(folder)
...
for folder in ['artist','fresh-verified','lifecycle-verified','controller-verified','audio-verified','solo-ui-verified','integrated-ui-03','bay-material-isolation','bay-final','lighting-final','full-art-review-02','baseline-districts-corrected','crew-ui-diagnosis','crew-ui-clock-repaired','live-audio-final-routing','day-verified','video-verified']:
 d=b/folder
 if d.exists():
  for p in d.glob('*'):
...
for n in ['root-fresh-720.png','build-link-720.png','crew-hud-1080.png']:add(b/'integrated-ui-03'/n)
...
for n in ['complete-race-LIVE-AUDIO.mp4','live-game-audio.webm','grid.png','contest.png','cockpit.png','final-lap.png','result.png','saved-build.png']:add(b/'video-verified'/n)
for n in ['garage-day-SILENT.mp4','day-time-trial.png']:add(b/'day-verified'/n)
...
manifest={'root':str(root),'runtimeCommit':build['commit'],'packagingCommit':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'runtimeInputManifest':str(b/'build-inputs-verified.json').replace('\\','/'),'files':{n:{'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}for n,p in sorted(files.items())}}
