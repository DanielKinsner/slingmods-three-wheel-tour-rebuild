...
try{
 ...
 await shot('earned-classification');assert.equal(await page.locator('.film-detail').textContent(),`${result.race.playerResult.place===1?'VICTORY':`P${result.race.playerResult.place} / FINISH`}  /  ${(result.race.playerResult.timeMs/1000).toFixed(3)} SEC`);
