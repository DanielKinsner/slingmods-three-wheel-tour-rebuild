"""Cut only loading/sync markers from actual live captures; preserve real-speed audio/video."""
import json, subprocess, sys, array, math, hashlib
from pathlib import Path
root=Path(sys.argv[1]); video=Path(json.loads((root/'video-path.json').read_text())['path'])
...
for i,s in enumerate(segments):
 ...
 duration=end-start-.45; offset=.25
...
final=root/'P08B-Signature-Experience.mp4';subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-f','concat','-safe','0','-i',str(concat),'-c:v','copy','-af','aresample=async=1:first_pts=0','-c:a','aac','-b:a','128k','-ar','48000','-movflags','+faststart',str(final)],check=True)
...
report={'pass':True,'method':'Real-speed edited demonstration, with only loading and disclosed sync-marker boundaries cut. Actual live game graph audio, measured recorded chirps aligned to captured video flashes. No synthesized replacement audio or time warping. Audio listening approval remains human.','excludedSegments':{name:'Both video sync flashes absent from captured frames; full original video/audio retained. Optional Original Harbor excerpt omitted rather than claiming unverified synchronization.' for name in excluded},'cuts':cuts,'video':str(video),'film':str(final),'duration':duration,'bytes':final.stat().st_size,'sha256':hashlib.sha256(final.read_bytes()).hexdigest(),'decodedAudio':{'peak':peak,'rms':rms,'samples':len(pcm)},'probe':probe}
