"""Purpose-built P03A material-inspection bay; authored geometry and exportable texture maps."""
import bpy, math, json
import numpy as np
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'public/assets'; MAPS=OUT/'textures/bay'; MAPS.mkdir(parents=True,exist_ok=True)
...
def mat(name,color,rough=.7,metal=0,emission=0):
 ...
 return m
floor=mat('Bay_microcement',(.34,.35,.34),.73);wall=mat('Bay_charcoal',(.055,.065,.073),.82);edge=mat('Bay_anodized_frame',(.1,.12,.14),.35,.6);light=mat('Bay_broad_fixture',(.9,.92,1),.4,0,4);accent=mat('SlingMods_C91820',(.584,.0091,.0144),.5);letter=mat('Bay_lettering',(.62,.66,.69),.5,.1)
...
def image(name,rgb,data=False):
 ...
 return im
base=np.clip(.58+noise*.009,0,1);baseim=image('microcement_base',np.stack([base*.98,base,base*.99],axis=-1))
...
box('rear_wall',(0,-4.8,2.25),(14,.18,4.5),wall,.025)
box('wall_shadow_gap',(0,-4.67,.14),(14,.05,.06),edge,.007)
for x in [-5.4,-2.7,2.7,5.4]:box('wall_panel_joint',(x,-4.697,2.25),(.008,.01,4.35),edge)
box('restrained_red_accent',(-5.8,-4.685,1.85),(.045,.022,1.5),accent,.003)
for x in [-2.4,0,2.4]:
 box('overhead_frame',(x,0,3.9),(1.8,5.2,.095),edge,.025)
 box('overhead_diffuser',(x,0,3.845),(1.64,5.02,.025),light,.035)
# Text is actual Blender mesh, not a composited image or web overlay.
for name,text,size,loc,m in [('brand','SLINGMODS',.33,(-2.0,-4.68,2.55),letter),('bay_name','MATERIAL  /  INSPECTION',.095,(-2.0,-4.678,2.26),letter)]:
...
s['purpose']='P03A compact material inspection bay, not final interactive garage';s['maps']='Authored deterministic microcement maps, 512px, 0.6m repeat; tangent +Y normals'
path=ROOT/'assets/blender/inspection-bay-p03a.blend';bpy.ops.wm.save_as_mainfile(filepath=str(path));bpy.ops.export_scene.gltf(filepath=str(OUT/'inspection-bay-p03a.glb'),export_format='GLB',export_apply=True,export_extras=True)
