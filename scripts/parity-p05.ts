...
if(process.argv.includes('--baseline')){writeFileSync(dir+'solo-before.json',bytes);console.log(JSON.stringify({rows:rows.length,hash}));}
else {const before=readFileSync(dir+'solo-before.json','utf8');if(bytes!==before)throw Error('Exact one-car parity failed');writeFileSync(dir+'parity.json',JSON.stringify({exact:true,rows:rows.length,hash,fixture:'scripts/parity-p05.ts'},null,2));console.log('EXACT PARITY',rows.length,hash);}
