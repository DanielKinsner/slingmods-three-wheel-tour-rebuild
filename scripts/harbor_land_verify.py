...
P=pathlib.Path(__file__).resolve().parents[1];E=P/'director-kit/production/evidence/P04A/artist'
def read(p):
...
def image_hashes(g,b):
 ...
 return sorted(out)
og,oldb=read(E/'dressed02/harbor.glb');g,b=read(P/'public/assets/harbor/harbor.glb');op,np=bymat(og),bymat(g);checks=[]
for name,p in np.items():
 if name=='Harbor_Land':continue
 ...
 for key in q['attributes']:
  a=acc(og,oldb,q['attributes'][key]);c=acc(g,b,p['attributes'][key]);assert[a[i]for i in qi]==[c[i]for i in pi],(name,key)
 checks.append(name)
assert image_hashes(og,oldb)==image_hashes(g,b)
assert (E/'dressed02/route.json').read_bytes()==(P/'public/assets/harbor/route.json').read_bytes()
...
report={'status':'PASS exported GLB corridor removal and unchanged non-land/map/route proof; matched runtime visual check separate','exactNonLandTriangleAttributes':checks,'embeddedImagesExactUnchanged':len(g['images']),'entireRouteJSONByteIdentical':True,'exportedLandTriangles':len(faces),'exportedLandRuntimeBounds':bounds,'corridorRays':rays,'corridorLandHits':0,'outsideCorridorY0Rays':outside,'cutHalfWidth':8.5,'cutMargin':0,'glbSha256':hashlib.sha256((P/'public/assets/harbor/harbor.glb').read_bytes()).hexdigest()}
(E/'dressed03-export-validation.json').write_text(json.dumps(report,indent=2));print(json.dumps(report))
