...
report=dict(status='Dressed02 shadow-policy separation and board-facing correction; awaiting runtime review' if DRESSED else 'Undressed route01 awaiting real-physics lap before dressing',length=length,samples=n,width=11,runoff=3,minimumFilletRadius=min(effective),filletRadii=effective,initialStraightMetres=200,colliderCount=len(coll),checkpoints=len(gates),triangles=sum(g['accessors'][p['indices']]['count']//3 for m in g['meshes']for p in m['primitives']),primitives=sum(len(m['primitives'])for m in g['meshes']),files={str(p.relative_to(P)):{'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'bytes':p.stat().st_size}for p in [B/'harbor.blend',O/'harbor.glb',O/'route.json']})
(E/('dressed02-export.json' if DRESSED else 'route01-export.json')).write_text(json.dumps(report,indent=2));print(json.dumps(report))
