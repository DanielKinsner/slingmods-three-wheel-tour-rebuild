import {createHarborRenderer,harborDepth} from '../src/presentation/harbor-renderer';
import * as THREE from 'three';
import {GLTFLoader} from 'three/addons/loaders/GLTFLoader.js';
import {loadOutdoorEnvironment} from '../src/presentation/outdoor-environment';
...
void(async()=>{
 ...
 const renderer=createHarborRenderer(true,params.get('depth')==='log');renderer.setPixelRatio(1);renderer.setSize(innerWidth,innerHeight);renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFShadowMap;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=preset==='night'?1.05:.95;document.body.style.margin='0';document.body.append(renderer.domElement);
 const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(38,innerWidth/innerHeight,.06,1600),loader=new GLTFLoader();await loadOutdoorEnvironment(scene,renderer,preset);
 ...
 function pose(distance:number,id:keyof typeof rivals,spacing:number,view:'near'|'far'|'cockpit',kit:'stock'|'cyan'|'red',showSwatch=false,district=false){
 ...
 function inspect(){return{config,renderDepth:harborDepth(renderer),showcase:showcase?.inspect(),render:{...renderer.info.render},memory:{...renderer.info.memory},programs:renderer.info.programs?.length,lighting:lights.inspect(),lightBudget:lights.budget,product:product.inspect(),drawingBuffer:renderer.getDrawingBufferSize(new THREE.Vector2()).toArray(),method:'Staged calibration fixture using exact runtime hero/driver/product/harbor and normal DrivingCamera math; stationary pose placement disclosed, never driving/physics/performance proof.'}}
 pose(0,'jett',8,'near','stock');const preparation=await product.prepare(()=>prepareRenderer(renderer,scene,camera));
 function diagnostic(eye:number[],target:number[],fov=45){camera.position.fromArray(eye);camera.lookAt(new THREE.Vector3().fromArray(target));camera.fov=fov;camera.updateProjectionMatrix();lights.update(new THREE.Vector3().fromArray(target),0);showcase?.update(camera.position,0);renderer.render(scene,camera);return{...inspect(),diagnostic:{eye,target,fov,method:'Runtime detail/overview camera; not normal driving'}};}
 (window as any).__VISUAL={ready:true,pose,diagnostic,inspect,preparation,diagnoseWater:(visible:boolean)=>{showcase?.root.traverse(o=>{if(o.userData.module==='water')o.visible=visible});renderer.render(scene,camera);return inspect()},depthRange:(far:number)=>{camera.far=far;camera.updateProjectionMatrix();renderer.render(scene,camera);return{near:camera.near,far:camera.far}},depthCapabilities:()=>({reversed:renderer.capabilities.reversedDepthBuffer,extensions:renderer.getContext().getSupportedExtensions(),near:camera.near,far:camera.far}),objects:()=>{const rows:any[]=[];showcase?.root.traverse(o=>{if(o instanceof THREE.Mesh&&/water|shoreland|landscape|apron/.test(o.name))rows.push({name:o.name,visible:o.visible,box:new THREE.Box3().setFromObject(o).min.toArray(),max:new THREE.Box3().setFromObject(o).max.toArray(),side:(o.material as THREE.Material).side})});return rows}};
})();
