...
foreach($case in $cases){
 $env:EVIDENCE_DIR='director-kit/production/evidence/P06/'+$case.Name
 $env:WIDTH=$case.Width;$env:EQUIPPED=$case.Equipped;$env:RACES=$case.Races;$env:SEED=$case.Seed;$env:RECORD='0'
 & node scripts/profile-review11.mjs | Tee-Object -FilePath ('director-kit/production/evidence/P06/'+$case.Name+'.log')
