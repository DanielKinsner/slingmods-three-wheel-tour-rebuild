...
for o in bpy.data.objects:
 ...
 uv_degenerate=None
 if m.uv_layers:
  uv=np.empty(len(m.loops)*2,dtype=np.float32);m.uv_layers.active.data.foreach_get('uv',uv);uv=uv.reshape(-1,2);ids=np.array([t.loops[:] for t in m.loop_triangles]);a=uv[ids[:,1]]-uv[ids[:,0]];b=uv[ids[:,2]]-uv[ids[:,0]];uv_degenerate=int(np.count_nonzero(np.abs(a[:,0]*b[:,1]-a[:,1]*b[:,0])<1e-12))
 report['objects'].append({'name':o.name,'vertices':len(m.vertices),'polygons':len(m.polygons),'triangles':len(m.loop_triangles),'collapsed_uv_triangles':uv_degenerate,'bounds':[co.min(axis=0).tolist(),co.max(axis=0).tolist()],'origin':list(o.location),'modifiers':[x.type for x in o.modifiers],'materials':[{'slot':i,'name':o.material_slots[i].name,'color':list(o.material_slots[i].material.diffuse_color)} for i in used],'uv_layers':len(m.uv_layers)})
