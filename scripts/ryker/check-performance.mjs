...
import {frameStatistics,frameVerdict,FRAME_LIMITS} from '../perf/frame-statistics.mjs';
...
const bundle=await build({configFile:false,logLevel:'silent',publicDir:false,build:{write:false,minify:false,lib:{entry:path.resolve('scripts/crew-evidence-agent.ts'),name:'CrewEvidence',formats:['iife']}}});
...
const report={startedAt:new Date().toISOString(),quality,cadence,route,look,url:base,browser:b.version(),args,vehicle:'can-am-ryker-900',gatePolicy:'Measurement of this addition only; existing Phase 2 HOLD remains unless the complete High/Ultra closing gate separately passes.',sourceCommit:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),
 thresholds:FRAME_LIMITS,method:'Isolated Chromium ANGLE D3D11 on owner GPU, native wall-clock RAF (uncapped lane explicitly disables vsync); 2560x1440 DPR1. Two complete races and at least 90 active seconds, bounded at three races. Vehicle-aware evidence driver through ordinary pedal/steering controls; real simulation/rivals/results. Audio graph active, output muted. All raw phases/outliers retained. No screenshots, readback, trace, draw wrappers or recorder in scored frames. CPU submission is not GPU execution time. OS/driver caches not flushed.',errors:[],failedRequests:[],finals:[],attempts:[]};
...
let heartbeat;
try {
 await p.goto(base+`/?scene=express&route=${route}&mode=race&play=preview&visual=ryker&test=1&profile=1&quality=${quality}&look=${look}`,{timeout:120000});
 ...
 assert.equal(report.initial.pipeline.quality,quality);assert.equal(report.initial.pipeline.post,true);assert.equal(report.initial.handlingProfile,'ryker-road-v1');assert.equal(report.initial.free,false);
 ...
 await p.evaluate(()=>{window.__drive=true;window.__finished=0;let attempt='',driver,completedId='',lastTick=-1;
  function frame(){if(!window.__drive)return;const h=window.__EXPRESS,s=h.lightweight();
   if(attempt!==s.attemptId){attempt=s.attemptId;driver=new window.CrewEvidence.EvidenceDriver(h.route);lastTick=-1;}
   if(s.ticks!==lastTick||s.race.phase!=='running'){h.setDeviceSample(s.race.phase==='running'?driver.sample(s.telemetry,s.field,1/60):window.CrewEvidence.toDevice());lastTick=s.ticks;}
   if(s.race.allFinished&&completedId!==s.attemptId){window.__finished++;completedId=s.attemptId;}
   requestAnimationFrame(frame);
  }requestAnimationFrame(frame);
 });
 heartbeat=setInterval(async()=>{try{console.log(JSON.stringify(await p.evaluate(()=>{const s=window.__EXPRESS.lightweight();return {phase:s.race.phase,elapsedMs:s.race.elapsedMs,finished:window.__finished,ticks:s.ticks,allFinished:s.race.allFinished,result:s.race.playerResult,standings:s.race.standings.map(x=>({id:x.id,status:x.status}))}})))}catch{}},30000);
...
}catch(error){report.failure=error.stack;report.failureState=await p.evaluate(()=>window.__EXPRESS?.inspect()).catch(()=>null);report.profile=await p.evaluate(()=>window.__EXPRESS?.profile()).catch(()=>null);await fs.writeFile(out+'/run.json',JSON.stringify(report));throw error}
