...
const out=process.env.EVIDENCE_DIR||'assets/ryker/evidence/complete/career';await mkdir(out,{recursive:true});
...
try{
 await p.goto((process.env.BASE_URL||'http://127.0.0.1:5198')+'/?profile=1&scene=bay&play=career&visual=ryker&test=1&captureBuffer=1');await ready();await capture('garage-stock');
