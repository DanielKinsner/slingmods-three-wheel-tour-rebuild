...
def copy(src,rel):
 dest=delivery/rel;dest.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(src,dest)
def sha(path):
...
for name in ['static-smoke.json','tests-final.log','demo-build.log','workflow-run.log','format-validation.log']:
...
receipt={'created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'branch':git('branch','--show-current'),'head':head,'main_base':base,'merge_tree':merge,'merge_conflicts':False,'tested_runtime':json.loads((repo/'demo-current.json').read_text())['commit'],'source_sha256':sha(workspace/'source/Blender 2.9 .blend'),'glb_sha256':sha(repo/'public/assets/ryker/ryker-900.glb'),'tests':396,'gltf_validator':{'errors':0,'warnings':0},'published':False,'physics_changed':False,'limits':'Shared Slingshot collision contacts; source width mismatch; rigid rear mechanics; blank source display; short performance samples only.'}
