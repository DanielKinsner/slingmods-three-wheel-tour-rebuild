...
copy(repo/'assets/ryker/MODS.md','MODS.md')
copy(repo/'assets/ryker/Ryker-Accessories.blend','editable/Ryker-Accessories.blend')
for path in (work/'mods').iterdir():
 if path.is_file():copy(path,'mods/'+path.name)
...
for name in ['static-smoke.json','tests-final.log','demo-build.log','workflow-run.log','format-validation.log','fidelity-workflow.log','fidelity-renders.log','finish-workflow.log','finish-renders.log']:
...
patch=subprocess.check_output(['git','-C',str(repo),'diff',base,'HEAD','--','src','tests/ryker.test.ts','tests/ryker-mods.test.ts','scripts/ryker','demo-assets.json','public/assets/ryker/*.json'])
