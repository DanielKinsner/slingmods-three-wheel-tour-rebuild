...
async function category(v){await p.locator('[data-action="category"][data-value="'+v+'"]').click();await wait(350)}async function action(a,v){const control=p.locator('[data-action="'+a+'"]'+(v?'[data-value="'+v+'"]':'')).first();if(!await control.isVisible()&&(['lighting','motion','ignition','driver'].includes(a)||(a==='view'&&v==='tour-wall')))await p.locator('[data-action=advanced]').click();await control.click();if(await p.evaluate(()=>!!window.__SIGNATURE))await p.waitForFunction(()=>!window.__SIGNATURE||!window.__SIGNATURE.inspect().pending);await wait(350)}
