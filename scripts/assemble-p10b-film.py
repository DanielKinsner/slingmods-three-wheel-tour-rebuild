...
for i,s in enumerate(segments):
 ...
 subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-ss',str(start+offset),'-i',str(video),'-ss',str(max(0,astart+offset)),'-i',str(audio),'-t',str(duration),'-map','0:v:0','-map','1:a:0','-c:v','libx264','-preset','medium','-crf','24','-maxrate','1250k','-bufsize','2500k','-vf',segment_label,'-pix_fmt','yuv420p','-r','25','-c:a','aac','-b:a','128k','-ar','48000','-movflags','+faststart',str(part)],check=True)
...
final=root/'P10B-Cinematic-Identity.mp4';subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-f','concat','-safe','0','-i',str(concat),'-c:v','copy','-af','aresample=async=1:first_pts=0','-c:a','aac','-b:a','128k','-ar','48000','-movflags','+faststart',str(final)],check=True)
