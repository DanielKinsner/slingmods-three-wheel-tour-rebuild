...
else {const before=readFileSync(dir+'solo-before.json','utf8');if(bytes!==before)throw Error('Exact one-car parity failed');writeFileSync('director-kit/production/evidence/P06/parity.json',JSON.stringify({exact:true,rows:rows.length,hash,fixture:'scripts/parity-p05.ts'},null,2));console.log('EXACT PARITY',rows.length,hash);}
