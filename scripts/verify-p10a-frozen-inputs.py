...
import json,hashlib
root=Path(__file__).resolve().parents[1];frozen=root/'director-kit/production/evidence/P10A/frozen-build-inputs.json';data=json.loads(frozen.read_text(encoding='utf-8'));checks=[]
...
(root/'director-kit/production/evidence/P10A/runtime-inputs-final.json').write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8');assert result['pass'];print(json.dumps({'pass':True,'checked':len(checks),'runtimeCommit':data['commit']}))
