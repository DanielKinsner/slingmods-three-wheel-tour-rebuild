...
const clean=(s=freshCareer())=>transition(s,{type:'award',event:'harbor',valid:true,id:randomUUID(),timeMs:78000}).state;
...
function v1(s:Career){const old=structuredClone(s)as any;old.version=1;delete old.crew;return old}
test('v1 fresh, earned, installed and unequipped careers migrate repeatably with exact old fields',()=>{
 ...
 for(const fixture of[freshCareer(),clean(),installed,{...installed,equipped:false}]){const old=v1(fixture),source=structuredClone(old),next=migrateCareer(old);assert.equal(next.version,2);assert.deepEqual(v1(next),source);assert.deepEqual(old,source);assert.deepEqual(migrateCareer(next),next);assert.equal(next.crew.cleared,false)}
});
