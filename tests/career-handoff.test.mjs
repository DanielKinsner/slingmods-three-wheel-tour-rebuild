...
test('denied-IDB real scene navigation preserves session career and rejects unsupported transfers',async()=>{
 ...
 const assertHistoricalWithBlankRidge=actual=>{const copy=structuredClone(actual);assert.deepEqual(copy.ownBuild.ridge,{completed:{'find-the-ridge':false,'nico-ridge-duel':false,'summit-invitational':false},won:{'find-the-ridge':false,'nico-ridge-duel':false,'summit-invitational':false}});delete copy.ownBuild.ridge;assert.deepEqual(copy,expected)};
 ...
 const check=async()=>{const actual=await page.evaluate(()=>({state:window.client.state,durable:window.client.durable,name:window.name,warning:document.querySelector('#career-storage').textContent}));assertHistoricalWithBlankRidge(actual.state);assert.equal(actual.durable,false);assert.equal(actual.name,'test-tab');assert.match(actual.warning,/Session only.*cannot be saved/)};
 await check();
 ...
 // Ridge uses the same explicit denied-storage transfer; neither credits nor old fields change.
 await page.evaluate(()=>window.client.navigate('?scene=ridge'));await page.waitForURL('**/?scene=ridge');await page.waitForFunction(()=>window.ready);await check();
 await page.evaluate(()=>window.client.navigate('?scene=career'));await page.waitForURL('**/?scene=career');await page.waitForFunction(()=>window.ready);await check();
 ...
 const allowed=await page.evaluate(()=>{const a=document.createElement('a');a.href='/?scene=crew';a.addEventListener('click',e=>e.preventDefault());document.body.append(a);a.click();a.remove();return JSON.parse(window.name.slice('twt-career-transfer:'.length))});assert.equal(allowed.target,'/?scene=crew');assertHistoricalWithBlankRidge(allowed.state);assert.equal(allowed.previousName,'unrelated-window-name');await context.close();
