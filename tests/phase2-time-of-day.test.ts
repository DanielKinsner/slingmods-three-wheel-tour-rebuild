...
test('Day and Night are the untouched validated looks; every look sits on one of the two validated bases',()=>{
 ...
 assert.equal(LOOKS['after-rain'].wet,true);assert.ok(LOOK_ORDER.filter(id=>id!=='after-rain').every(id=>!LOOKS[id].wet));
});
test('showcase default is Dusk on the flat routes; the owner can change and the choice is remembered per route',()=>{
 assert.deepEqual(DEFAULT_LOOK,{express:'dusk',harbor:'dusk'});assert.equal(flat().id,'dusk');assert.equal(flat({route:'harbor'}).id,'dusk');
 const store=memory();assert.equal(rememberLook('express','golden-hour',store),true);assert.equal(flat({storage:store}).id,'golden-hour');assert.equal(flat({route:'harbor',storage:store}).id,'dusk','per route');assert.deepEqual([...store.data.keys()],[LOOK_KEY]);assert.notEqual(LOOK_KEY,SAVE_KEY,'not part of the game save');
 assert.equal(flat({search:'?look=after-rain',storage:store}).id,'after-rain','a link wins');assert.equal(flat({search:'?look=banana'}).id,'dusk');assert.equal(flat({storage:memory({[LOOK_KEY]:'{broken'})}).id,'dusk');assert.equal(flat({storage:{getItem(){throw Error('blocked')}}}).id,'dusk');
