...
/** `area` marks extra drivable ground off the course (the Gymkhana Lot): being there is not "stranded". */
export function recoveryReason(t:VehicleTelemetry,route:CourseRoute,area?:(x:number,z:number)=>boolean){
 ...
 if(speed<1.5&&!area?.(t.position.x,t.position.z)&&projectRoad(route,t.position.x,t.position.z).distance>route.width/2+route.runoff+4)return 'Stranded off the course';
...
export class CrashRecovery {
 ...
 constructor(parent:Element,private route:CourseRoute,private free:boolean,onRecover:()=>void,private area?:(x:number,z:number)=>boolean){
 ...
 update(t:VehicleTelemetry,dt:number,eligible:boolean){const reason=eligible?recoveryReason(t,this.route,this.area):'';this.held=reason?this.held+Math.min(.1,dt):0;this.reason=reason;this.shown=this.held>=1.25;this.node.hidden=!this.shown}
