...
export class InputResolver {
 ...
 suspend(){this.holdNotBefore=this.lastNow;this.paused=true;this.armed=false;this.holdStart=undefined;this.holdLatched=false;this.baselineNext=true}
 disconnect(){this.readyPads.clear();this.connected.clear();this.prior.clear();this.activity.clear();this.suspend()}
 reset(){this.holdNotBefore=this.lastNow;this.reverse=false;this.armed=false;this.holdStart=undefined;this.holdLatched=true}
 poll(now:number,sample:DeviceSample){
  ...
  if(owner!==this.activeDevice){this.activeDevice=owner;this.holdNotBefore=now;this.holdStart=undefined;this.holdLatched=false}
  ...
  if(rise('pause')){this.paused=!this.paused;this.armed=false;this.holdStart=undefined;this.holdLatched=false}
  ...
  if(!this.paused&&this.armed){camera=rise('camera');if(rise('direction'))this.reverse=!this.reverse;
 ...
 private output(i:Intent,camera:boolean,reset:boolean){const enabled=this.armed&&!this.paused;const control:VehicleControl={throttle:enabled?i.throttle:0,brake:enabled?i.brake:0,steer:enabled?i.steer:0,reverse:this.reverse,tractionControl:true};return {control,paused:this.paused,armed:this.armed,activeDevice:this.activeDevice,status:this.status,camera,reset,lookBack:enabled&&i.actions.lookBack,resetProgress:this.holdStart===undefined?0:Math.min(1,(this.lastNow-this.holdStart)/INPUT_TUNING.resetHoldMs)}}
}
