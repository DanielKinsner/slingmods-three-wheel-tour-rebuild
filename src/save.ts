...
export interface Save { version:1; vehicleId:'slingshot-r-2024'; settings:{camera:'near'|'far'|'cockpit';mute:boolean;volume:number;interfaceVolume:number;engineVolume:number;musicVolume:number;environmentVolume:number};records:Record<string,LapRecord> }
export const freshSave = ():Save => ({version:1,vehicleId:'slingshot-r-2024',settings:{camera:'near',mute:false,volume:.55,interfaceVolume:1,engineVolume:1,musicVolume:.45,environmentVolume:.8},records:{}});
...
export function decodeSave(raw:string|null):Save {
 ...
 for(const key of ['interfaceVolume','engineVolume'] as const)out.settings[key]=Number.isFinite(v.settings?.[key])?Math.max(0,Math.min(1,v.settings[key])):1;
 for(const [key,fallback]of [['musicVolume',.45],['environmentVolume',.8]] as const)out.settings[key]=Number.isFinite(v.settings?.[key])?Math.max(0,Math.min(1,v.settings[key])):fallback;
...
export function browserStorage():Pick<Storage,'getItem'|'setItem'>{try{const q=typeof location==='undefined'?new URLSearchParams():new URLSearchParams(location.search);if(q.get('play')==='preview'||!['career','demo'].includes(q.get('play')??'')&&(['signature','express','ridge'].includes(q.get('scene')??'')||!q.has('scene'))){const storage=window.sessionStorage;return {getItem:()=>storage.getItem('slingmods-signature-settings-v1'),setItem:(_key,value)=>storage.setItem('slingmods-signature-settings-v1',value)}}if(activeProfile()==='demo'){const storage=window.sessionStorage;return {getItem:()=>storage.getItem(DEMO_SETTINGS_KEY),setItem:(_key,value)=>storage.setItem(DEMO_SETTINGS_KEY,value)}}return window.localStorage}catch{return {getItem:()=>null,setItem:()=>{throw new Error('Storage unavailable')}}}}
