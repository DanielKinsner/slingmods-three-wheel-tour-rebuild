...
const diagnostics=document.createElement('button');diagnostics.className='diagnostics-toggle';diagnostics.textContent=bay?'Inspection tools':'Diagnostics';diagnostics.addEventListener('click',()=>document.body.classList.toggle('debug-overlay'));document.querySelector('#app')!.append(diagnostics);
...
if(bay){
 ...
 chapterUI=new ChapterUI(career,{build:()=>buildUI?.open(),shakedown:()=>{chapterUI?.setVisible(false);soloEntry.hidden=false;soloEntry.querySelector<HTMLAnchorElement>('#start-shakedown')?.focus()},race:()=>career.navigate('?scene=crew')});
