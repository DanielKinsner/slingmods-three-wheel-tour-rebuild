...
let chapterUI:ChapterUI|undefined;let buildUI:BuildUI|undefined;const syncProduct=()=>product?.set(career.state.equipped,career.state.appearance);syncProduct();const unsubCareer=career.subscribe(()=>{if(!buildUI)syncProduct()});
...
if(bay){
 ...
 buildUI=new BuildUI(career,{open:opened=>{chapterUI?.setVisible(!opened);soloEntry.hidden=true;controls.enabled=!opened;if(opened){normalCamera={position:camera.position.clone(),target:controls.target.clone(),fov:camera.fov};fitBuild()}else if(normalCamera){camera.clearViewOffset();camera.aspect=innerWidth/innerHeight;camera.fov=normalCamera.fov;camera.position.copy(normalCamera.position);controls.target.copy(normalCamera.target);camera.lookAt(controls.target);camera.updateProjectionMatrix();fitBuild();render()}},previewNight:enabled=>{normalLights.forEach((power,light)=>light.intensity=power*(enabled?.24:1));scene.environmentIntensity=enabled?.24:normalEnvironment;render()},appearance:(equipped,appearance)=>{product?.set(equipped,appearance);render()}});button.onclick=()=>buildUI?.open();
 chapterUI=new ChapterUI(career,{build:()=>buildUI?.open(),shakedown:()=>{chapterUI?.setVisible(false);soloEntry.hidden=false;soloEntry.querySelector<HTMLAnchorElement>('#start-shakedown')?.focus()},race:()=>career.navigate('?scene=crew')});if(query.get('shop')==='build')buildUI.open();
