...
import {ChaseCamera} from './presentation/chase';
...
const loader=new GLTFLoader();const vehicleFile=proofAsset??(query.get('asset')==='p03a2'?'slingshot-p03a2.glb':query.get('asset')==='fleet'?'scale-blockouts.glb':query.get('asset')==='p01'?'slingshot.glb':query.get('asset')==='p03a'?'slingshot-p03a.glb':query.get('asset')==='p03a1'?'slingshot-p03a1.glb':'slingshot-p03a2.glb');
...
if(['slingshot-p03a1.glb','slingshot-p03a2.glb'].includes(vehicleFile))asset.scene.traverse(o=>{if(o instanceof THREE.Mesh)for(const m of (Array.isArray(o.material)?o.material:[o.material]))if(m instanceof THREE.MeshStandardMaterial&&m.name.includes('Radar_Blue')&&m.map){m.map.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());m.map.needsUpdate=true}});
...
let current=sim?.telemetry(),previous=current;let accumulator=0,manual=query.has('test')&&!query.has('clock'),paused=false,cap=Number(query.get('cap')||60),last=performance.now(),lastDraw=last;let far=loadSave(localStorage).settings.camera==='far',reverse=false,resetHeld=0;
...
const fidelity=vehicleFile==='slingshot-p03a1.glb'||vehicleFile==='slingshot-p03a2.glb',proof=!!proofAsset,candidate=fidelity||proof||vehicleFile==='slingshot-p03a.glb';
document.querySelector('#stage')!.textContent=(proof?'P03A2 / LOCAL PROOF':bay?(fidelity?(vehicleFile==='slingshot-p03a2.glb'?'P03A2 / INSPECTION':'P03A1 / INSPECTION'):'P03A / INSPECTION'):driving?'P03B1 / PRACTICE':'NEUTRAL / INSPECTION')+' · '+__BUILD_REF__;
...
const assetQuery=vehicleFile==='slingshot-p03a2.glb'?'&asset=p03a2':(fidelity||proof)?'&asset=p03a1':candidate?'&asset=p03a':vehicleFile==='scale-blockouts.glb'?'&asset=fleet':'&asset=p01';
...
if(driving){const help=document.createElement('details');help.id='driving-controls';help.innerHTML='<summary>Controls & practice route</summary><p>Launch straight → gentle slalom → broad sweeper → stop box.<br>Cones are visual guides.</p><p>Keyboard: W/S throttle/brake · A/D steer · C near/far · hold B look back · X direction · hold R one second to reset · Esc pause.</p><p>Controller: RT/LT throttle/brake · left stick steer · top face camera · LB look back · right face direction · hold bottom face reset · Menu pause.</p>';document.querySelector('#app')!.append(help);
...
function cameraChase(dt:number,snap=false){
 chase.update(vehicle.position,vehicle.quaternion,current?.speed??0,dt,far,lookBack,snap,(target,desired)=>{const ray=desired.clone().sub(target);cameraRay.set(target,ray.clone().normalize());cameraRay.far=ray.length();return cameraRay.intersectObject(pad.scene,true)[0]?.distance});
}
...
function suspendDriving(){keyboard.clear();session?.input.suspend();paused=!!session;accumulator=0;updateHUD()}
