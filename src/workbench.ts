...
function render(){
 ...
 suspension?.update();product?.update();renderer.render(scene,camera);
}
...
const gameAudio=new GameAudio(document.querySelector('#app')!);let audioFrame:ReturnType<GameAudio['update']>|undefined;
...
addEventListener('resize',()=>{if(!pageActive())return;camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);if(buildUI?.inspect().open)fitBuild();draw(1/60)});addEventListener('pagehide',()=>{chapterUI?.dispose();buildUI?.dispose();unsubCareer();career.close();suspension?.dispose();product?.dispose();sim?.dispose();if(pad.scene.userData.showcase)disposeShowcaseBay(pad.scene);scene.environment=null;inspectionProbe.dispose();renderer.dispose();controls.dispose()});
