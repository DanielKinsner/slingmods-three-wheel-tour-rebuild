...
const pmrem=new THREE.PMREMGenerator(renderer),env=new RoomEnvironment();scene.environment=pmrem.fromScene(env,.04).texture;env.dispose();pmrem.dispose();scene.environmentIntensity=bay?.85:.55;scene.environmentRotation.y=bay?.4:0;
...
const loader=new GLTFLoader();const vehicleFile=query.get('asset')==='fleet'?'scale-blockouts.glb':query.get('asset')==='p01'?'slingshot.glb':'slingshot-p03a.glb';
const [asset,pad]=await Promise.all([loader.loadAsync('/assets/vehicles/'+vehicleFile),loader.loadAsync(bay?'/assets/inspection-bay-p03a.glb':'/assets/test-pad.glb')]);
...
const candidate=vehicleFile==='slingshot-p03a.glb';
document.querySelector('#stage')!.textContent=(bay?'P03A / INSPECTION':driving?'P02 / DRIVING PAD':'NEUTRAL / INSPECTION')+' · '+__BUILD_REF__;
...
const assetQuery=candidate?'&asset=p03a':vehicleFile==='scale-blockouts.glb'?'&asset=fleet':'&asset=p01';
...
function view(name:string){
 const presets:Record<string,[number,number,number]>={front:[0,.9,-7.6],rear:[0,1,7.6],side:[7.5,.9,0],threequarter:[5,3.3,-6],rearquarter:[4.6,2.8,6],cockpit:[-.55,2.2,1.2],material:[2.3,1.15,-2.2],bay:[4.2,2.4,-6.3],fleet:[8,5,-11]};const p=presets[name]??presets.threequarter;camera.position.set(...p);controls.target.set(name==='material'?.55:0,name==='material'?.52:.6,name==='material'?-1.25:0);camera.lookAt(controls.target);controls.update();renderer.render(scene,camera);
}
function updateHUD(){
 if(!current){document.querySelector('#status')!.textContent=candidate?'P03A candidate · '+(bay?'Inspection bay':'Neutral inspection')+' · G3 pending':'P01 clay baseline · Neutral runtime inspection';hud.innerHTML='<span class="metric">'+(candidate?'P03A / CANDIDATE':'P01 / CLAY')+'</span><p>'+renderer.info.render.calls+' renderer calls · '+renderer.info.render.triangles.toLocaleString()+' submitted triangles</p>';return}
