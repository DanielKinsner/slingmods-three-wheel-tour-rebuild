...
function render(){
 // Keep inspection views in front of the bay's rear wall (front face z=4.71m).
 // Preserve the viewing ray while dollying in; this also protects the Rear preset and zoom/orbit.
 if(bay&&camera.position.z>4.45){const offset=camera.position.clone().sub(controls.target);camera.position.copy(controls.target).addScaledVector(offset,(4.45-controls.target.z)/offset.z);camera.lookAt(controls.target)}
