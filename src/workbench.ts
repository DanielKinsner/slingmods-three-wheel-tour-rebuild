...
document.querySelector('#stage')!.textContent=(proof?'P03A2 / LOCAL PROOF':bay?(fidelity?(vehicleFile===CURRENT_VEHICLE?'P04A1 / INSPECTION':'P03A1 / INSPECTION'):'P03A / INSPECTION'):driving?'P03B2 / FIRST DRIVE':'NEUTRAL / INSPECTION')+' · '+__BUILD_REF__;
...
function updateHUD(){
 if(!current){document.querySelector('#status')!.textContent=candidate?(proof?'P03A2 diagnostic, one brow · ':fidelity?(vehicleFile===CURRENT_VEHICLE?'P04A1 candidate · ':'P03A1 candidate · '):'P03A candidate · ')+(bay?'Inspection bay':'Neutral inspection')+' · G3 pending':'P01 clay baseline · Neutral runtime inspection';hud.innerHTML='<span class="metric">'+(candidate?(proof?'P03A2 / LOCAL PROOF':fidelity?(vehicleFile===CURRENT_VEHICLE?'P04A1 / CANDIDATE':'P03A1 / CANDIDATE'):'P03A / CANDIDATE'):'P01 / CLAY')+'</span><p>'+renderer.info.render.calls+' renderer calls · '+renderer.info.render.triangles.toLocaleString()+' submitted triangles</p>';return}
