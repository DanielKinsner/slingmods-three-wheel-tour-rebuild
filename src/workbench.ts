...
import {RenderPipeline,SHOWROOM_BLOOM,DEFAULT_BLOOM} from './presentation/render-pipeline';
import {loadGraphicsQuality,GRAPHICS_PRESETS} from './presentation/graphics-settings';
import {installGraphicsControl} from './presentation/graphics-control';
import {CURRENT_HANDLING_PROFILE} from './simulation/profile';
import {careerRecipe} from './career-experience/model';
import {restoreConsoleDetail} from './presentation/console-detail';
import {FrontLinks} from './presentation/front-links';
...
document.body.classList.toggle('player-bay',bay&&!query.has('neutral'));document.body.classList.toggle('inspection-bay',bay);document.body.classList.toggle('debug-overlay',query.has('debug'));
...
const graphics=loadGraphicsQuality();
...
const surfaceFinish=await loadSurfaceMaterials(renderer,pad.scene,bay&&!query.has('neutral')&&query.get('surfaces')!=='off');
if(vehicleFile===CURRENT_VEHICLE)restoreConsoleDetail(asset.scene);
...
const finish=vehicleFile===CURRENT_VEHICLE?new SignatureFinishPresenter(asset.scene,{studio:true}):undefined;
const accessories=vehicleFile===CURRENT_VEHICLE?await SignatureProducts.load(loader,asset.scene):undefined;
const frontLinks=vehicleFile===CURRENT_VEHICLE?new FrontLinks(asset.scene):undefined;
const mirrors=vehicleFile===CURRENT_VEHICLE?new VehicleMirrors(vehicle,asset.scene):undefined;
const display=vehicleFile===CURRENT_VEHICLE?new PoweredDisplay(asset.scene):undefined;
const pipeline=new RenderPipeline(renderer,graphics);pipeline.onViewTarget(target=>{if(mirrors)mirrors.viewTarget=target});
const graphicsControl=installGraphicsControl(document.querySelector('#app')!,renderer,pipeline,GRAPHICS_PRESETS[graphics].lighting);if(bay)graphicsControl.root.style.display='flex';
await finish?.set(career.state.ownBuild.finish);accessories?.set(Object.keys(careerRecipe(career.state).products));display?.snapshot(renderer,scene.environment,JSON.stringify(careerRecipe(career.state)));
let chapterUI:ChapterUI|undefined;let buildUI:BuildUI|undefined;const syncProduct=()=>product?.set(career.state.equipped,career.state.appearance);syncProduct();const unsubCareer=career.subscribe(()=>{if(!buildUI)syncProduct();accessories?.set(Object.keys(careerRecipe(career.state).products));void finish?.set(career.state.ownBuild.finish).then(()=>{display?.snapshot(renderer,scene.environment,JSON.stringify(careerRecipe(career.state)));render()})});
...
let sim=driving?await Simulation.create(undefined,CURRENT_HANDLING_PROFILE):null;
sim?.configureSuspension(career.state.suspension.equipped?career.state.suspension.setup:null);
const parkedSim=!driving&&vehicleFile===CURRENT_VEHICLE?await Simulation.create(undefined,CURRENT_HANDLING_PROFILE):null,parkedTelemetry=parkedSim?.telemetry();parkedSim?.dispose();
...
const diagnostics=document.createElement('button');diagnostics.className='diagnostics-toggle';diagnostics.textContent=bay?'Inspection tools':'Diagnostics';diagnostics.addEventListener('click',()=>document.body.classList.toggle('debug-overlay'));if(activeProfile()!=='demo'&&evidenceEnabled())document.querySelector('#app')!.append(diagnostics);
...
function pose(t:any){
 ...
 frontLinks?.update(t.wheels);if(rearPresenter)rearPresenter.update(t.wheels[2]);
...
function updateHUD(){
 if(!current&&bay&&!query.has('neutral')){document.querySelector('#status')!.textContent='YOUR BUILD / SLINGMODS';hud.innerHTML='<p>Inspection tools · '+renderer.info.render.calls+' renderer calls · '+renderer.info.render.triangles.toLocaleString()+' submitted triangles</p>';return}
...
function normalFrame(now:number,present=true){
 buildUI?.frame(readDevices());chapterUI?.frame(readDevices());if(!session)return;const result=session.frame(now);inputState=result;current=result.current;previous=result.previous;accumulator=result.alpha/60;paused=result.paused;reverse=result.control.reverse;lookBack=result.lookBack;
...
addEventListener('resize',()=>{if(!pageActive())return;camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setPixelRatio(Math.min(devicePixelRatio,GRAPHICS_PRESETS[pipeline.quality].pixelRatioCap));renderer.setSize(innerWidth,innerHeight);pipeline.resize();if(buildUI?.inspect().open)fitBuild();draw(1/60)});addEventListener('pagehide',()=>{surfaceFinish.dispose();chapterUI?.dispose();buildUI?.dispose();finish?.dispose();accessories?.dispose();display?.dispose();mirrors?.dispose();pipeline.dispose();graphicsControl.dispose();unsubCareer();career.close();suspension?.dispose();product?.dispose();sim?.dispose();if(pad.scene.userData.showcase)disposeShowcaseBay(pad.scene);scene.environment=null;inspectionProbe.dispose();renderer.dispose();controls.dispose()});
