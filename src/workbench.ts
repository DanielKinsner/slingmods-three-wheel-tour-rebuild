...
let activeScenario:DrivingScenario|undefined;
...
const debug={ready:true,playScenario:(name:string)=>{const selected=scenarios.find(s=>s.name===name);if(!selected||!sim)throw new Error('Unknown driving scenario');activeScenario=selected;sim.reset(selected.pose);current=sim.telemetry();previous=current;manual=false;paused=false;accumulator=0;draw(1/60,true);return selected.seconds},mode:driving?'pad':'vehicle',asset:vehicleFile,view,inspect:()=>({bounds:new THREE.Box3().setFromObject(asset.scene).getSize(new THREE.Vector3()).toArray(),nodes:bindings.map(b=>({id:b.id,steer:!!b.steer,spin:!!b.spin,position:b.basePos?.toArray()})),telemetry:current??null,steeringControlQuaternion:steeringControl?.quaternion.toArray(),cameraPosition:camera.position.toArray(),renderInfo:renderer.info.render,renderer:renderer.getContext().getParameter(renderer.getContext().RENDERER)}),telemetry:()=>current,reset:(p?:any)=>{activeScenario=undefined;sim?.reset(p);current=sim?.telemetry();previous=current;accumulator=0;draw(1/60,true)},advance:(control:typeof input,seconds:number)=>{manual=true;for(let i=0;i<Math.round(seconds*60);i++)step(control);draw(seconds,true);return current},renderFrame:(control:typeof input,delta:number,present=true)=>{manual=true;accumulator+=delta;while(accumulator+1e-10>=1/60){step(control);accumulator-=1/60}if(present)draw(delta);return current},captureView:view,setCamera:(name:string)=>{far=name==='far';draw(1/60,true)}};(window as any).__TWT=debug;
