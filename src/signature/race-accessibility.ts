...
export function installRaceMenuFocus(root:HTMLElement){
 const onKey=(event:KeyboardEvent)=>{
  ...
  const choices=[...menu.querySelectorAll<HTMLElement>('button:not(:disabled),a[href],summary'),...document.querySelectorAll<HTMLElement>('#game-audio button:not(:disabled),#game-audio input')].filter(e=>!e.hidden&&!!e.getClientRects().length);
