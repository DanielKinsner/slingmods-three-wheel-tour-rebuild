...
export interface BuildRecipe {ryker?:RykerBuild;version:1|2;vehicleId:'slingshot-r-2024'|'can-am-ryker-900';definitionId?:'ryker-900-v1';finish:FinishId;products:Partial<Record<ProductId,string>>;lights:Appearance;suspension:SuspensionSetup;handlingProfile:'slingmods-sport-v1'|'slingmods-sport-v2'|'slingmods-sport-v3'|'slingmods-sport-v4'|'slingmods-sport-v5'|'ryker-road-v1'}
...
export function validateRecipe(raw:unknown):BuildRecipe {
 ...
 if(!r||!((r.version===1&&r.vehicleId==='slingshot-r-2024'&&r.handlingProfile!=='ryker-road-v1')||(r.version===2&&r.vehicleId==='can-am-ryker-900'&&r.definitionId==='ryker-900-v1'&&r.handlingProfile==='ryker-road-v1'&&Object.keys(r.products??{}).length===0))||!['slingmods-sport-v1','slingmods-sport-v2','slingmods-sport-v3','slingmods-sport-v4','slingmods-sport-v5','ryker-road-v1'].includes(r.handlingProfile)||!FINISHES.some(f=>f.id===r.finish)||!validSetup(r.suspension)||!r.products||typeof r.products!=='object'||Array.isArray(r.products)||!r.lights||!Object.hasOwn(COLORS,r.lights.color)||typeof r.lights.enabled!=='boolean'||!Number.isFinite(r.lights.brightness)||r.lights.brightness<.15||r.lights.brightness>.85)throw Error('This build recipe needs recovery; saved data is unchanged.');
...
export const driveTuneLabel=(id:BuildRecipe['handlingProfile'])=>id==='ryker-road-v1'?'Ryker Road v1 · CVT':id===CURRENT_HANDLING_PROFILE?'Sport v5 · responsive':id==='slingmods-sport-v4'?'Sport v4 · forgiving reference':id==='slingmods-sport-v3'?'Sport v3 · historical':id==='slingmods-sport-v2'?'Sport v2 · historical':'Sport v1 · historical';
...
export function currentDrivingCopy(recipe:BuildRecipe):BuildRecipe{return {...validateRecipe(recipe),handlingProfile:recipe.vehicleId==='can-am-ryker-900'?'ryker-road-v1':CURRENT_HANDLING_PROFILE}}
...
export class BuildRepository {
 ...
 private resolve(recipe:BuildRecipe){const r=validateRecipe(recipe);if(this.vehicle==='ryker')return rykerRecipe(r);if(r.vehicleId!=='slingshot-r-2024')throw Error('Switch to the Ryker to load this build.');return r}
 private fresh(){return this.vehicle==='ryker'?freshRykerRecipe():freshRecipe()}
