...
export interface BuildRecipe {version:1;vehicleId:'slingshot-r-2024';finish:FinishId;products:Partial<Record<ProductId,string>>;lights:Appearance;suspension:SuspensionSetup;handlingProfile:'slingmods-sport-v1'|'slingmods-sport-v2'|'slingmods-sport-v3'|'slingmods-sport-v4'|'slingmods-sport-v5'}
...
export class BuildRepository {
 ...
 beginDrive(recipe:BuildRecipe,route:DriveSnapshot['route'],mode:DriveSnapshot['mode'],lighting:DestinationLighting='day'){const snapshot=previewSnapshot(recipe,route,mode,lighting);this.session.setItem(DRIVE_KEY,JSON.stringify(snapshot));return snapshot}
 drive(route:DriveSnapshot['route']='express',mode:DriveSnapshot['mode']='test'):DriveSnapshot{const raw=this.session.getItem(DRIVE_KEY);if(!raw)return previewSnapshot(freshRecipe(),route,mode);const s=JSON.parse(raw);if(s.version!==1||s.scope!=='preview'||!['express','harbor','ridge'].includes(s.route)||(s.lighting!==undefined&&!['day','night'].includes(s.lighting))||!['test','race'].includes(s.mode)||s.returnTo!=='?scene=signature&screen=build')throw Error('Preview drive snapshot is invalid. Return to the showroom.');return {...s,recipe:validateRecipe(s.recipe)}}
}
