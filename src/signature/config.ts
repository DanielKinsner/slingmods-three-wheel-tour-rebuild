...
export interface BuildRecipe {ryker?:RykerBuild;version:1;vehicleId:'slingshot-r-2024';finish:FinishId;products:Partial<Record<ProductId,string>>;lights:Appearance;suspension:SuspensionSetup;handlingProfile:'slingmods-sport-v1'|'slingmods-sport-v2'|'slingmods-sport-v3'|'slingmods-sport-v4'|'slingmods-sport-v5'}
...
export const driveTuneLabel=(id:BuildRecipe['handlingProfile'])=>id===CURRENT_HANDLING_PROFILE?'Sport v5 · responsive':id==='slingmods-sport-v4'?'Sport v4 · forgiving reference':id==='slingmods-sport-v3'?'Sport v3 · historical':id==='slingmods-sport-v2'?'Sport v2 · historical':'Sport v1 · historical';
/** Product-only comparison: never changes the chosen driving equations. */
export function stockRecipe(recipe:BuildRecipe):BuildRecipe{return {...validateRecipe(recipe),products:{},...(recipe.ryker?{ryker:{}}:{}),suspension:streetSetup()}}
...
export function buildSummary(r:BuildRecipe){
 if(CURRENT_VEHICLE_CONTEXT.visual==='ryker')return ['Can-Am Ryker 900 — game preview',...RYKER_PRODUCTS.filter(p=>r.ryker?.[p.id]).map(p=>`${p.brand} ${p.name} (${p.sku})\n${p.fit}\n${p.url}`),'Reference-based game meshes. Verify your exact year/trim before purchasing.'].join('\n\n');
...
export class BuildRepository {
 constructor(private durable:Pick<Storage,'getItem'|'setItem'>,private session:Pick<Storage,'getItem'|'setItem'>,private vehicle=CURRENT_VEHICLE_CONTEXT.visual==='ryker'?'ryker':'slingshot'){}
 private key(key:string){return this.vehicle==='ryker'?key+'-ryker':key}
 ...
 draft(){const raw=this.session.getItem(this.key(DRAFT_KEY));return raw?validateRecipe(JSON.parse(raw)):freshRecipe()}
 setDraft(recipe:BuildRecipe){this.session.setItem(this.key(DRAFT_KEY),JSON.stringify(validateRecipe(recipe)))}
 beginDrive(recipe:BuildRecipe,route:DriveSnapshot['route'],mode:DriveSnapshot['mode'],lighting:DestinationLighting='day'){const snapshot=previewSnapshot(recipe,route,mode,lighting);this.session.setItem(this.key(DRIVE_KEY),JSON.stringify(snapshot));return snapshot}
 drive(route:DriveSnapshot['route']='express',mode:DriveSnapshot['mode']='test'):DriveSnapshot{const raw=this.session.getItem(this.key(DRIVE_KEY));if(!raw)return previewSnapshot(freshRecipe(),route,mode);const s=JSON.parse(raw);if(s.version!==1||s.scope!=='preview'||!['express','harbor','ridge'].includes(s.route)||(s.lighting!==undefined&&!['day','night'].includes(s.lighting))||!['test','race'].includes(s.mode)||s.returnTo!=='?scene=signature&screen=build')throw Error('Preview drive snapshot is invalid. Return to the showroom.');return {...s,recipe:currentDrivingCopy(s.recipe)}}
}
