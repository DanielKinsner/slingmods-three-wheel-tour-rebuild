...
resize();await apply();view('hero',false);const preparation=await prepareRenderer(renderer,scene,camera);veil.remove();const loadMs=performance.now()-started;
...
function inspect(){return {simulationProfile,display:display.inspect(),ready:true,drivePreparation,departure:departure?.inspect()??null,showroom:{tourWall:wall.inspect(),look:STUDIO_LOOKS[lighting],safeRegion:ui.safeRegion(),view:currentView,driverRendered:hero.driver.root.visible,curtainPosition:room.getObjectByName('bay_door_curtain')?.position.toArray(),curtainScale:room.getObjectByName('bay_door_curtain')?.scale.toArray()},build:__BUILD_REF__,recipe:structuredClone(recipe),original,finish:finishes.inspect(),compare,lighting,destinationLighting,driverVisible,reducedMotion,status,pending,screen,saved,loadMs,preparation,frames,memory:{...renderer.info.memory},calls:renderer.info.render.calls,triangles:renderer.info.render.triangles,products:accessories.inspect?.(),suspension:shocks.inspect(),audio:audio.inspect(),simulation:simulation.telemetry(),profile:simulation.suspensionConfig(),camera:{position:camera.position.toArray(),target:controls.target.toArray()},bounds3d:visibleBounds(hero.root),bounds:projectedBounds(camera,visibleBounds(hero.root)),resources:performance.getEntriesByType('resource').map((r:any)=>({name:r.name,bytes:r.transferSize,duration:r.duration})),intervals}}
...
addEventListener('pagehide',()=>{departure?.dispose();wall.dispose();contact.dispose();probe.dispose();display.dispose();audio.dispose();simulation.dispose();controls.dispose();under.dispose();shocks.dispose();accessories.dispose();finishes.dispose();renderer.dispose()});
