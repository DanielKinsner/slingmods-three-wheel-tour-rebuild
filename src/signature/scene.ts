...
simulation.reset({x:0,z:0,y:0,yaw:0});if(recipe.vehicleId==='can-am-ryker-900')simulation.configureRykerSuspension(!!recipe.ryker?.shocks);for(let i=0;i<120;i++)simulation.step({throttle:0,brake:1,steer:0,reverse:false});
...
async function apply(){
 ...
 simulation.reset({x:0,z:0,y:0,yaw:0});simulation.configureSuspension(shown.products['SM-3223']?shown.suspension:null);if(shown.vehicleId==='can-am-ryker-900')simulation.configureRykerSuspension(!!shown.ryker?.shocks);for(let i=0;i<90;i++)simulation.step({throttle:0,brake:1,steer:0,reverse:false});neutral=simulation.telemetry();hero.pose(neutral,0,false,true);hero.driver.root.visible=driverVisible;contact.ground(hero.root,()=>0);shocks.update();
...
async function action(name:string,value?:unknown){if(pending)return;try{
 ...
 // Back: leave an inspection first, then retrace in-app history, then go up one level. Never toggles.
  ...
  departure=new ShowroomDeparture({parent:app,car:hero.root,room,camera,target:controls.target,wheelRadius:recipe.vehicleId==='can-am-ryker-900'?.2848:undefined,wheelbase:recipe.vehicleId==='can-am-ryker-900'?1.709:undefined,
   onWheelPose:(spin,steer,speed)=>{if(recipe.vehicleId==='can-am-ryker-900'&&'pose' in hero.rear){const t={...neutral,steer,speed,rpm:1050+Math.min(3800,speed*900),throttle:speed>.01?.35:0,brake:0,wheels:neutral.wheels.map((w,i)=>({...w,spin,steer:i<2?steer:0,longitudinalSpeed:speed}))};departureTelemetry=t;hero.rear.pose(t);hero.driver.update(t,1/60,false)}else hero.rear.update({...neutral.wheels[2],spin})},onSoundStart:()=>audio.startBayDoor(),onSoundStop:()=>{audio.stopBayDoor();audio.stopDeparture()},onSoundFrame:(elapsed,paused)=>audio.updateDeparture(elapsed,paused),
