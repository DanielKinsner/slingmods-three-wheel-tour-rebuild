...
import {RenderPipeline} from '../presentation/render-pipeline';import {loadGraphicsQuality,GRAPHICS_PRESETS} from '../presentation/graphics-settings';
...
function render(){if(!departure)constrainCamera();mirrors.update(camera,innerHeight,!departure&&(currentView==='interior'||currentView==='cockpit'));mirrors.render(renderer,scene,camera);under.update();shocks.update();optics.update(0,ignition);display.update(neutral,camera,performance.now());pipeline.render(scene,camera)}
...
controls.addEventListener('start',()=>{cameraTween=undefined});let virtual:DeviceSample|undefined;addEventListener('keydown',e=>keyboard.down(e.code,performance.now(),e.repeat));addEventListener('keyup',e=>keyboard.up(e.code));
...
addEventListener('pagehide',()=>{pipeline.dispose();mirrors.dispose();departure?.dispose();wall.dispose();contact.dispose();probe.dispose();optics.dispose();display.dispose();audio.dispose();simulation.dispose();controls.dispose();under.dispose();shocks.dispose();accessories.dispose();finishes.dispose();renderer.dispose()});
