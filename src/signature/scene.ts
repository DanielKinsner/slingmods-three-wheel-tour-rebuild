import {disposeKTX2} from '../presentation/ktx2';
...
import {challengeById} from '../game/challenges';
...
import {InstallMotion,drawnSet} from '../presentation/install-motion';
...
import {Simulation,type VehicleTelemetry} from '../simulation';
...
const [under,shocks,accessories]=await Promise.all([ProductPresenter.load(loader,hero.root,scene),SuspensionPresenter.load(loader,hero.asset),SignatureProducts.load(loader,hero.asset)]),finishes=new SignatureFinishPresenter(hero.asset,{studio:true}),audio=new GameAudio(app),install=new InstallMotion(scene);
...
let focusPull:number|undefined,focusFreeze:number|undefined;const FOCUS_PULL_MS=1100;
...
function careerSummary():CareerSummary{const s=careerState(),step=nextCareerStep(s);return {label:step.kind==='start'?'Start Career':'Continue Career',detail:step.detail,returning:!!returnHref,returnLabel:fromCareer==='bay'?'Back to career garage':'Back to career',temporary:!!careerView?.temporary||!!career&&!career.durable,earned:!!returnHref&&!!s&&sameBuild(recipe,careerRecipe(s)),progress:tourProgress(s)}}
...
async function apply(){
 const shown=compare?stockRecipe(recipe):recipe;accessories.inspectStorage(false);shocks.inspectionView(null);await finishes.set(shown.finish);accessories.set(Object.keys(shown.products),shown.ryker,shown.spyder,shown.lights);const glow=garageOverride.under??[hero.asset.getObjectByName('ryker_foundation')?!!shown.ryker?.underglow:!!shown.products['SM-133'],shown.lights];under.set(glow[0],glow[1]);shocks.set(garageOverride.shocks??!!shown.products['SM-3223']);shocks.update();under.update();hero.driver.root.visible=driverVisible;audio.setExhaustTreatment(!!shown.products['SM-7720']);
 // Recreate only the static diagnostic pose when setup changes; actual drives capture the recipe at entry.
 simulation.reset({x:0,z:0,y:0,yaw:0});if(shown.spyder)simulation.configureSpyder(shown.spyder);simulation.configureSuspension(shown.products['SM-3223']?shown.suspension:null);if(shown.vehicleId==='can-am-ryker-900')simulation.configureRykerSuspension(!!shown.ryker?.shocks);for(let i=0;i<90;i++)simulation.step({throttle:0,brake:1,steer:0,reverse:false});neutral=simulation.telemetry();hero.pose(neutral,0,false,true);hero.driver.root.visible=driverVisible;contact.ground(hero.root,()=>0);shocks.update();
...
async function change(next:BuildRecipe,preset=false){if(pending||garage)return;try{const verified=validateRecipe(next),cue=buildCue(recipe,verified,preset);if(!cue)return;const lights=cue==='part.lights.attach'||!!verified.spyder?.parts.underglow&&!recipe.spyder?.parts.underglow,quiet=compare||reducedMotion||!(cue.startsWith('part.')&&cue!=='part.remove'||cue==='ui.detent');storage.store.setDraft(verified);history.push(structuredClone(recipe));history=history.slice(-24);recipe=verified;historyFragment();compare=false;status='Preview updated · career credits unchanged';pending=true;refresh();install.finish();const before=drawnSet(hero.root);await apply();
  // A part that appears on the car flies in (lights ignite); its sound and stamp land with it. Spyder parts share the detent sound.
  const feedback=(moved:boolean)=>{audio.cue(cue);if(moved)setTimeout(()=>stamp('INSTALLED','Preview build · no career spend'),260);else if(cue.startsWith('part.')&&cue!=='part.remove')stamp('INSTALLED','Preview build · no career spend');else if(cue==='part.remove')stamp('REMOVED','','red');else if(cue==='finish.apply')stamp('NEW FINISH','','good');else if(cue==='build.preset')stamp('PRESET LOADED','','good')};
  if(quiet||!install.play(hero.root,before,{now:performance.now(),camera,lights,exclude:hero.driver.root,onLand:()=>feedback(true)}))feedback(false);pending=false;refresh()}catch(e){pending=false;status=(e as Error).message;refresh()}}
...
// Rack focus: on a camera move or a new category the focus starts behind the car, pulls onto it, then the lens effect
// fades out so the resting image is unchanged. Uses the pipeline's existing film lens (depth blur only at this amount).
function pullFocus(){if(!reducedMotion&&!departure)focusPull=performance.now()}
function focusLens(now:number){if(focusFreeze===undefined&&(focusPull===undefined||departure||reducedMotion))return undefined;const u=focusFreeze??(now-focusPull!)/FOCUS_PULL_MS;if(u>=1){focusPull=undefined;return undefined}
 const subject=camera.position.distanceTo(controls.target),rack=THREE.MathUtils.smoothstep(u,0,.5),open=u<.12?u/.12:1-THREE.MathUtils.smoothstep(u,.55,1);
 return {amount:.002,focus:subject*(1+1.25*(1-rack)),aperture:1.25*open,time:0}}
async function reapply(){install.finish();pending=true;refresh();try{await apply()}finally{pending=false;refresh()}}
...
async function action(name:string,value?:unknown){if(pending)return;try{
 ...
 // Back: leave an inspection first, then retrace in-app history, then go up one level. Never toggles.
 ...
 else if(name==='test-drive'||name==='race'||name==='time-attack'||name==='challenge'){
  // A challenge is a test drive on its own course, started at the challenge line.
  const chal=name==='challenge'?challengeById(String(value)):undefined;if(name==='challenge'){if(!chal)return;value=chal.route;name='test-drive'}
  ...
  const target=`?scene=${route==='ridge'?'ridge':'express'}&route=${route}&mode=${name==='test-drive'?'test':'race'}${name==='time-attack'?'&trial=1':''}${chal?'&challenge='+chal.id:''}${seriesRun?'&series=1':''}&play=preview${route==='ridge'?'&lighting='+destinationLighting:'&look='+look}${fromCareer?'&from='+fromCareer:''}${recipeFragment(snapshot)}`;
  ...
  departure=new ShowroomDeparture({parent:app,car:hero.root,room,camera,target:controls.target,wheelRadius:recipe.spyder?.32355:recipe.vehicleId==='can-am-ryker-900'?.2848:undefined,wheelbase:recipe.vehicleId!=='slingshot-r-2024'?1.709:undefined,
   onWheelPose:(spin,steer,speed)=>{if(recipe.vehicleId!=='slingshot-r-2024'&&'pose' in hero.rear){const t={...neutral,steer,speed,rpm:1050+Math.min(3800,speed*900),throttle:speed>.01?.35:0,brake:0,wheels:neutral.wheels.map((w,i)=>({...w,spin,steer:i<2?steer:0,longitudinalSpeed:speed}))};departureTelemetry=t;hero.rear.pose(t);hero.driver.update(t,1/60,false)}else hero.rear.update({...neutral.wheels[2],spin})},onSoundStart:()=>audio.startBayDoor(),onSoundStop:()=>{audio.stopBayDoor();audio.stopDeparture()},onSoundFrame:(elapsed,paused)=>audio.updateDeparture(elapsed,paused),
...
function render(){const glow=install.update(performance.now());under.flare(glow);accessories.flare(glow);accessories.update(performance.now()/1000);if(!departure)containCamera(camera,controls.target,SHOWROOM_SAFE_VOLUME);mirrors.update(camera,innerHeight,!departure&&(currentView==='interior'||currentView==='cockpit'));mirrors.render(renderer,scene,camera);under.update();shocks.update();optics.update(0,ignition);display.update(departureTelemetry??neutral,camera,performance.now());pipeline.render(scene,camera,{bloom:SHOWROOM_BLOOM,cinema:focusLens(performance.now())})}
...
let last=performance.now(),frames=0,idleSway=0,idleAngle=0;const intervals:number[]=[];
function frame(now:number){if(!pageActive()){requestAnimationFrame(frame);return}const dt=Math.min(.1,(now-last)/1000);intervals.push(now-last);if(intervals.length>6000)intervals.shift();last=now;const input=virtual??{...keyboard.read(),pads:Array.from(navigator.getGamepads?.()??[]),focused:document.hasFocus()&&!document.hidden};if(departure)departure.frame(dt,{...input,focused:input.focused&&showroomFocused&&!document.hidden});else{ui.frame(input);const preset=presetLimits(currentView);if(cameraTween){const t=Math.min(1,(now-cameraTween.start)/800),u=t*t*(3-2*t);camera.position.lerpVectors(cameraTween.from,cameraTween.to,u);controls.target.lerpVectors(cameraTween.fromTarget,cameraTween.toTarget,u);if(t===1)cameraTween=undefined}
  // Main menu life: a slow cinematic sway around the car on the opening screen (never after a manual orbit, never with reduced motion).
  else if(!garage&&screen==='entry'&&currentView==='hero'&&!viewManual&&!orbiting&&!reducedMotion&&!controls.autoRotate){idleSway+=dt;const angle=Math.sin(idleSway*.26)*.22,delta=angle-idleAngle;idleAngle=angle;if(Math.abs(delta)>1e-6){const offset=camera.position.clone().sub(controls.target).applyAxisAngle(new THREE.Vector3(0,1,0),delta);camera.position.copy(controls.target).add(offset)}}
...
if(evidenceEnabled()){const api={ready:true,inspect,view,referenceCamera,focusFreeze:(u?:number)=>{if(!params.has('test'))throw Error('Isolated test required');focusFreeze=u},poseVisual:(steer:number,travel:number|number[],spin:number)=>{if(!params.has('test'))throw Error('Isolated test required');const t=structuredClone(neutral);t.steer=steer;t.wheels.forEach((w,i)=>{w.localCenter.y+=Array.isArray(travel)?travel[i]:travel;w.steer=i<2?steer:0;w.spin=spin});hero.pose(t,1/60,false,true);render();return hero.inspectVisual()},setDeviceSample:(s:any)=>{if(!params.has('test'))throw Error('Isolated test required');virtual=s?{...s,keys:new Set(s.keys)}:undefined},startAudioCapture:()=>audio.startEvidenceCapture(),audioSync:(id:string)=>audio.evidenceMarker(id),stopAudioCapture:()=>audio.stopEvidenceCapture()};(window as any).__SIGNATURE=api;
...
addEventListener('pagehide',()=>{surfaceFinish.dispose();pipeline.dispose();mirrors.dispose();departure?.dispose();wall.dispose();contact.dispose();probe.dispose();optics.dispose();display.dispose();audio.dispose();simulation.dispose();controls.dispose();under.dispose();shocks.dispose();accessories.dispose();finishes.dispose();graphicsControl?.dispose();careerView?.dispose();career?.close();disposeKTX2(renderer);renderer.dispose()});
