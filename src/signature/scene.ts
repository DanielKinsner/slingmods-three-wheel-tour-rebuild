...
import {buildRepository,fragmentRecipe,recipeFragment,validateRecipe,freshRecipe,stockRecipe,PRESETS,buildSummary,type BuildRecipe} from './config';
...
const loader=new GLTFLoader();const [hero,room]=await Promise.all([loadDrivingHero(loader),loadSignatureShowroom(loader)]);scene.add(hero.root,room);
...
let neutral=simulation.telemetry();const display=new PoweredDisplay(hero.asset);let ignition=true;hero.pose(neutral,0,false,true);hero.driver.root.visible=false;
...
const ui=new SignatureUI(app,{onChange:change,onAction:action,onSave:save,onNavigate:()=>audio.cue('ui.nav')});
let departure:ShowroomDeparture|undefined,navigating=false;let drivePreparation:DrivePreparationReport|undefined;
function state():SignatureState{return {ignition,recipe:structuredClone(recipe),pending,status,screen,lighting,driverVisible,compare,reducedMotion,canUndo:history.length>0,savedRecipes:saved.map(r=>({id:r.id,name:r.name})),ownedProductIds:[],thumbs,...routeGraphics}}
...
async function apply(){
 ...
 simulation.reset({x:0,z:0,y:0,yaw:0});simulation.configureSuspension(shown.products['SM-3223']?shown.suspension:null);for(let i=0;i<90;i++)simulation.step({throttle:0,brake:1,steer:0,reverse:false});neutral=simulation.telemetry();hero.pose(neutral,0,false,true);hero.driver.root.visible=driverVisible;shocks.update();
 const studio=lighting==='studio';renderer.toneMappingExposure=studio?.95:.82;scene.environmentIntensity=studio?.88:.24;hemi.intensity=studio?1.1:.38;key.intensity=studio?2.1:.55;fill.intensity=studio?.8:.25;rim.intensity=studio?.95:.4;scene.background=new THREE.Color(studio?'#777d80':'#252c34');display.snapshot(renderer,scene.environment,JSON.stringify(shown));refresh();
}
function view(name:string){controls.minDistance=name==='interior'||name==='cockpit'?.45:3;accessories.inspectStorage(false);shocks.inspectionView(null);const directions:Record<string,THREE.Vector3>={hero:new THREE.Vector3(5,2.5,-6),front:new THREE.Vector3(0,1.8,-7),rear:new THREE.Vector3(4,2.2,6),side:new THREE.Vector3(7,1.8,0)};
...
function restoreDeparture(error:unknown){
 navigating=false;audio.stopDeparture();audio.stopBayDoor();departure?.dispose();departure=undefined;keyboard.clear();ui.root.hidden=false;ui.root.inert=false;controls.enabled=true;hero.driver.root.visible=driverVisible;
...
async function action(name:string,value?:unknown){if(pending)return;try{
 if(name==='ignition'){ignition=value==='true';display.setPower(ignition);audio.cue('power')}
 ...
 else if(name==='quick-race'){audio.cue('ui.nav');screen='events'}
  ...
  departure=new ShowroomDeparture({parent:app,car:hero.root,room,camera,target:controls.target,
   onSoundStart:()=>audio.startBayDoor(),onSoundStop:()=>{audio.stopBayDoor();audio.stopDeparture()},onSoundFrame:(elapsed,paused)=>audio.updateDeparture(elapsed,paused),
...
function render(){if(!departure)constrainCamera();under.update();shocks.update();display.update(neutral,camera,performance.now());renderer.render(scene,camera)}
