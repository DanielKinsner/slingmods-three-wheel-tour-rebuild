...
export function showRecovery(kind:'assets'|'graphics'|'context'|'demo'){
 ...
 const heading=kind==='demo'?'This demo session needs a reset':kind==='assets'?'The harbor could not finish loading':kind==='context'?'Graphics connection interrupted':'WebGL 2 is unavailable';
