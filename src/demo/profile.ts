...
export function visitorSearch(search:string){const input=new URLSearchParams(search),out=new URLSearchParams();for(const [key,values]of Object.entries({visual:['josh','legacy','2026'],scene:['bay','crew','harbor','signature','express','career','ridge'],play:['demo','career','preview'],screen:['entry','build','events','shop'],route:['express','harbor','ridge'],lighting:['day','night'],mode:['test','race'],preset:['day','night'],quality:['standard','low'],shop:['build'],event:['duel']})){const value=input.get(key);if(value&&values.includes(value))out.set(key,value)}if(['express','ridge'].includes(input.get('scene')??'')&&input.get('play')==='career'&&/^[0-9a-f-]{36}$/i.test(input.get('attempt')??''))out.set('attempt',input.get('attempt')!);if(input.get('test')==='1'&&input.get('profile')==='1'){out.set('test','1');out.set('profile','1');for(const key of ['clock','captureBuffer','seed']){const value=input.get(key);if(value&&((key==='clock'&&value==='controlled')||(key==='captureBuffer'&&value==='1')||(key==='seed'&&/^\d{1,6}$/.test(value))))out.set(key,value)}}return out.toString()}
