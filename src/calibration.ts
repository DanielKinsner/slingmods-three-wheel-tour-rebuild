...
import {RenderPipeline,DEFAULT_BLOOM} from './presentation/render-pipeline';
import {loadGraphicsQuality,GRAPHICS_PRESETS} from './presentation/graphics-settings';
import {prepareRenderer} from './presentation/prepare';
...
const renderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});renderer.setSize(innerWidth,innerHeight);renderer.setPixelRatio(Math.min(devicePixelRatio,GRAPHICS_PRESETS[graphics].pixelRatioCap));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;
document.querySelector('#viewport')!.appendChild(renderer.domElement);
const pipeline=new RenderPipeline(renderer,graphics);
const render=()=>pipeline.render(scene,camera,{bloom:DEFAULT_BLOOM});
...
const preparation=await prepareRenderer(renderer,scene,camera,render);
const debug={ready:true,asset:'calibration.glb',save:saved,animationCount:gltf.animations.length,time:0,inspect:()=>{
 ...
 return {pipeline:pipeline.inspect(),preparation,axes:{origin:point('axis_origin'),x:point('axis_x'),y:point('axis_y'),z:point('axis_z')},cubeBounds:new THREE.Box3().setFromObject(patch).getSize(new THREE.Vector3()).toArray(),clearcoat:(patch.material as THREE.MeshPhysicalMaterial).clearcoat,rubberRoughness:((gltf.scene.getObjectByName('rubber_patch') as THREE.Mesh).material as THREE.MeshStandardMaterial).roughness,wheelQuaternion:gltf.scene.getObjectByName('wheel_spin')!.quaternion.toArray(),animationCount:gltf.animations.length,renderer:renderer.getContext().getParameter(renderer.getContext().RENDERER)};
},setTime:(t:number)=>{mixer.setTime(t);render()}};
...
let previous=performance.now();function frame(now:number){const dt=Math.min((now-previous)/1000,.05);previous=now;if(!document.hidden){mixer.update(dt);debug.time+=dt;controls.update();render()}requestAnimationFrame(frame)}requestAnimationFrame(frame);
addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setPixelRatio(Math.min(devicePixelRatio,GRAPHICS_PRESETS[pipeline.quality].pixelRatioCap));renderer.setSize(innerWidth,innerHeight);pipeline.resize()});
addEventListener('pagehide',()=>pipeline.dispose(),{once:true});
