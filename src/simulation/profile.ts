...
export type HandlingProfileId='legacy-p08a'|'slingmods-sport-v1'|'slingmods-sport-v2'|'slingmods-sport-v3'|'slingmods-sport-v4';
export const CURRENT_HANDLING_PROFILE='slingmods-sport-v4' as const;
...
export function steeringLimit(speed:number,id:HandlingProfileId='legacy-p08a',wheelbase=2.667){const p=handlingProfile(id);const geometric=Math.atan(p.lateralSteerLimit*wheelbase/Math.max(speed*speed,1));
 ...
 const reserve=(id==='slingmods-sport-v3'||id==='slingmods-sport-v4')?.025*speed*speed/(speed*speed+100)/(1+speed*speed/500):0;
