...
export class GameAudio {
 private filmScore?:RaceFilmScore;
 ...
 film(film:Film|null,elapsed=0){if(!this.filmScore&&this.context&&this.mix&&this.enabled)this.filmScore=new RaceFilmScore(this.context,this.mix);this.filmScore?.update(film,elapsed,this.enabled&&!this.settings.mute,this.settings.volume*this.settings.interfaceVolume)}
 ...
 lifecycle(paused:boolean){this.paused=paused;if(paused)this.filmScore?.stop();this.opponents?.silence();if(this.departureActive)this.thermal?.update(this.departureElapsed,paused,this.enabled&&!this.settings.mute,this.settings.volume*this.settings.engineVolume);this.refresh()}
