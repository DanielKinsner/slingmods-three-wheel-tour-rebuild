import {RaceFilmScore} from './race-film-score';
import type {Film} from '../presentation/cinematics/tracks';
import './tour-controls.css';
import {TourDirector,type SoundScene,DEFAULT_SOUND_SCENE} from './tour-director';
...
export class GameAudio {
 ...
 private enginePowered=true;private director?:TourDirector;private soundScene:SoundScene={...DEFAULT_SOUND_SCENE};private sportExhaust=false;private departureActive=false;private departureElapsed=0;
 ...
 constructor(parent:Element){
  ...
  const musicOff=document.createElement('button');musicOff.id='music-off';musicOff.textContent='Music off';musicOff.onclick=()=>{this.settings.musicVolume=0;const slider=settings.querySelector<HTMLInputElement>('#audio-music');if(slider){slider.value='0';slider.dispatchEvent(new Event('input'))}else{this.persist();this.refresh()}};settings.append(musicOff);this.ui.append(this.enable,this.mute,settings);parent.append(this.ui);this.enable.onclick=()=>void this.unlock();this.mute.onclick=()=>{this.settings.mute=!this.settings.mute;this.persist();this.refresh()};
 ...
 private persist(){const save=loadSave(browserStorage());Object.assign(save.settings,{mute:this.settings.mute,volume:this.settings.volume,interfaceVolume:this.settings.interfaceVolume,engineVolume:this.settings.engineVolume,musicVolume:this.settings.musicVolume,environmentVolume:this.settings.environmentVolume});writeSave(browserStorage(),save)}
 ...
 async unlock(){if(this.loading||this.disposed)return;this.loading=true;try{
  ...
  if(!this.graph){const bank=await loadAudioBank(this.context,'tour-engine-v1');if(this.disposed||this.context.state==='closed')return;this.mix=this.context.createGain();this.mix.gain.value=1;this.limiter=this.context.createDynamicsCompressor();this.limiter.threshold.value=-3;this.limiter.knee.value=3;this.limiter.ratio.value=12;this.limiter.attack.value=.003;this.limiter.release.value=.12;this.mix.connect(this.limiter);this.limiter.connect(this.context.destination);this.graph=createAudioGraph(this.context,bank,this.mix);this.bank=bank;this.interfaceAudio=new InterfaceAudio(this.context,this.mix);this.thermal=new ThermalDepartureAudio(this.context,this.mix);this.director=new TourDirector(this.context,this.mix);this.director.setScene(this.soundScene);this.director.setCinematic(!!this.currentFilm);await Promise.all([this.interfaceAudio.prepare(),this.thermal.prepare(),this.director.prepare()]);if(this.disposed)return}
 ...
 film(film:Film|null,elapsed=0){this.currentFilm=film;this.filmElapsed=elapsed;this.director?.setCinematic(!!film);this.refreshFilm()}
 ...
 beginDeparture(equipped:boolean){this.director?.duck(6);if(this.last?.powertrain!=='six-speed')this.director?.play(this.last?.powertrain==='cvt'?'ignition-ryker':'ignition-sling',.35);this.departureActive=true;this.departureElapsed=0;if(this.last?.powertrain!=='six-speed')this.thermal?.begin(equipped,this.enabled&&!this.settings.mute&&!this.paused);this.refresh()}
 ...
 lifecycle(paused:boolean){this.paused=paused;if(paused)this.film(null);this.opponents?.silence();if(this.departureActive)this.thermal?.update(this.departureElapsed,paused,this.enabled&&!this.settings.mute,this.settings.volume*this.settings.engineVolume);this.refresh()}
 private refresh(){this.refreshFilm();this.director?.update(this.life(),{environment:this.settings.environmentVolume,music:this.settings.musicVolume});const label=this.settings.mute?'Unmute':'Mute';if(this.mute.textContent!==label)this.mute.textContent=label;if(this.last)this.graph?.apply(mapAudio(this.last,this.engineLife()),false);this.bayDoor?.update(this.life());this.interfaceAudio?.update(this.enabled&&!this.paused&&!this.settings.mute,this.settings.volume*this.settings.interfaceVolume)}
 ...
 private hornVoice?:Horn;
 /** Horn (H / RB while driving). Follows the engine's volume, mute and pause. */
 horn(pressed:boolean){if(!this.context||!this.mix){return}if(!this.hornVoice){if(!pressed)return;this.hornVoice=new Horn(this.context,this.mix)}const life=this.engineLife();this.hornVoice.set(pressed&&life.enabled&&!life.paused&&!life.mute,life.volume)}
 private draft=0;
 /** Slipstream share (RaceWorld racecraft): the wind roar drops while tucked in behind another car. */
 setDraft(share:number){this.draft=share}
 private engineLife(reset=false):AudioLife{const duck=this.departureActive&&this.thermal?.active?0:1;return{...this.life(reset),draft:this.draft,volume:this.settings.volume*this.settings.engineVolume*duck*(this.enginePowered?1:0)*(this.soundScene.place==='showroom'&&!this.departureActive?.3:1)}}
 update(t:VehicleTelemetry,paused:boolean,cockpit:boolean,reset=false){this.last=t;this.paused=paused;this.cockpit=cockpit;this.director?.update(this.life(reset),{environment:this.settings.environmentVolume,music:this.settings.musicVolume},t);const life=this.engineLife(reset),parameters=mapAudio(t,life),shift=this.shifts.update(t,reset);this.graph?.setTreatment(this.sportExhaust);this.graph?.apply(parameters,shift);this.bayDoor?.update(this.life(reset));this.interfaceAudio?.update(this.enabled&&!paused&&!this.settings.mute,this.settings.volume*this.settings.interfaceVolume);return{life,parameters,shift}}
 inspect(){return{horn:this.hornVoice?.inspect()??null,cinematicScore:this.filmScore?.inspect(),enabled:this.enabled,loading:this.loading,failure:this.failure,soundTreatment:this.last?.powertrain==='six-speed'?'synthesized-inline-three-SE6':this.last?.powertrain==='cvt'?'tour-designed-three-cylinder':this.sportExhaust?'tour-designed-sport-mix':'tour-designed-stock-mix',life:this.life(),levels:{master:this.settings.volume,interface:this.settings.interfaceVolume,engine:this.settings.engineVolume},score:this.director?.inspect(),extendedLevels:{music:this.settings.musicVolume,environment:this.settings.environmentVolume},bayDoor:this.bayDoor?.inspect(),thermal:this.thermal?.inspect(),cues:this.interfaceAudio?.inspect(),opponents:this.opponents?.inspect(),graph:this.graph?.inspect(),captureBus:'post-limiter master including engine, rivals, interface, door, Thermal, world, road and score'}}
 private releaseGraph(){this.hornVoice?.dispose();this.hornVoice=undefined;this.filmScore?.dispose();this.filmScore=undefined;this.director?.dispose();this.director=undefined;this.bayDoor?.dispose();this.bayDoor=undefined;this.thermal?.dispose();this.thermal=undefined;this.interfaceAudio?.dispose();this.interfaceAudio=undefined;this.capture?.abort();this.capture=undefined;this.opponents?.dispose();this.opponents=undefined;this.graph?.dispose();this.graph=undefined;this.mix?.disconnect();this.limiter?.disconnect()}
 dispose(){this.disposed=true;this.releaseGraph();void this.context?.close();this.ui.remove()}
}
export async function renderOfflineGameAudio(timeline:AudioTimelineFrame[],duration:number,revision:'old'|'new'|'tour'='tour'){const ctx=new OfflineAudioContext(2,Math.round(duration*48000),48000),bank=await loadAudioBank(ctx,revision==='old'?'p03b2':revision==='new'?'p09b':'tour-engine-v1'),graph=createAudioGraph(ctx,bank),events=new ShiftEvents();for(const f of timeline)graph.apply((revision==='old'?mapAudioLegacy:mapAudio)(f.telemetry,f.life),events.update(f.telemetry,!!f.life.reset),f.time);const rendered=await ctx.startRendering();const pcm=new ArrayBuffer(44+rendered.length*4),v=new DataView(pcm);const text=(offset:number,s:string)=>[...s].forEach((x,i)=>v.setUint8(offset+i,x.charCodeAt(0)));text(0,'RIFF');v.setUint32(4,pcm.byteLength-8,true);text(8,'WAVEfmt ');v.setUint32(16,16,true);v.setUint16(20,1,true);v.setUint16(22,2,true);v.setUint32(24,48000,true);v.setUint32(28,192000,true);v.setUint16(32,4,true);v.setUint16(34,16,true);text(36,'data');v.setUint32(40,rendered.length*4,true);let peak=0,nonfinite=0,clipped=0;for(let i=0;i<rendered.length;i++)for(let ch=0;ch<2;ch++){const x=rendered.getChannelData(ch)[i];if(!Number.isFinite(x))nonfinite++;peak=Math.max(peak,Math.abs(x));if(Math.abs(x)>=1)clipped++;v.setInt16(44+(i*2+ch)*2,Math.round(Math.max(-1,Math.min(1,x))*32767),true)}let binary='';const bytes=new Uint8Array(pcm);for(let i=0;i<bytes.length;i+=32768)binary+=String.fromCharCode(...bytes.subarray(i,i+32768));return{wavBase64:btoa(binary),samplePeak:peak,nonfinite,clipped,duration,method:'OfflineAudioContext using identical runtime graph factory, source buffers, mapper and full logical timeline; not live hardware capture'}}
