...
export class OpponentAudio {
 ...
 constructor(private ctx:BaseAudioContext,bank:AudioBank,output:AudioNode=ctx.destination){this.slots=Array.from({length:2},()=>{const pan=ctx.createStereoPanner();pan.connect(output);return {pan,graph:createAudioGraph(ctx,bank,pan),id:'',changed:0}})}
