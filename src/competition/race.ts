...
export interface CrewResult {event:string;attemptId:string;participantId:string;status:CrewStatus;valid:boolean;laps:number;timeMs:number|null;place:number|null;invalidReason:string|null}
...
export class CrewRace {
 ...
 constructor(readonly route:CourseRoute,readonly participantIds:readonly string[]=['maya','jett','nico','player'],readonly playerId='player',readonly duel=false,readonly custom?:{eventId?:string;laps?:1|2;handlingProfileId:string;freeDrive?:boolean;timeTrial?:boolean}){if(custom&&(custom.eventId!==undefined&&!custom.eventId||!custom.eventId&&(custom.laps!==undefined||custom.freeDrive||custom.timeTrial)))throw Error('Custom laps or free drive require a separate event identity');const count=custom?.freeDrive||custom?.timeTrial?1:duel?2:4;if(participantIds.length!==count||new Set(participantIds).size!==count||!participantIds.includes(playerId))throw Error('Crew race requires four distinct participants including player');this.road=new RaceRoad(route);this.initialize()}
 get laps(){return this.custom?.laps??(this.duel?1:2)} get event(){return this.custom?.eventId??(this.duel?DUEL_EVENT:CREW_EVENT_ID)}
 ...
 tick(previous:Record<string,VehicleTelemetry>,current:Record<string,VehicleTelemetry>,dt:number){
  ...
  const before=this.elapsedMs;this.elapsedMs+=dt*1000;if(this.custom?.freeDrive)return;const candidates:Participant[]=[];
 ...
 snapshot(){const states=[...this.states.values()].sort((a,b)=>{if(a.status==='finished'||b.status==='finished')return a.status==='finished'&&b.status==='finished'?a.place!-b.place!:a.status==='finished'?-1:1;if(a.status==='running'!== (b.status==='running'))return a.status==='running'?-1:1;return b.progress-a.progress||a.id.localeCompare(b.id)});
  ...
  return {event:this.event,handlingProfileId:this.custom?.handlingProfileId??'legacy-p08a',laps:this.laps,phase:this.phase,paused:this.paused,countdown:this.phase==='countdown'?Math.max(0,3-this.countdownTicks/60):0,elapsedMs:this.elapsedMs,attemptId:this.attemptId,standings,playerResult,allFinished:states.every(s=>s.status!=='running')};
 }
}
