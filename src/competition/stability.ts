...
export function stabilizeRival(v:VehicleTelemetry,control:VehicleControl,profileId:HandlingProfileId='legacy-p08a'):VehicleControl {
 ...
 return {...control,throttle:0,brake:Math.max(control.brake,.4),steer:(profileId==='slingmods-sport-v3'||profileId==='slingmods-sport-v4'||profileId==='slingmods-sport-v5')?steeringDemandForAngle(angle,v.speed,profileId,Math.max(control.brake,.4)):Math.max(-1,Math.min(1,angle/maxSteer))};
}
