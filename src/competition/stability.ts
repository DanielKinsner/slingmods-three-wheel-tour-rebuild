...
export function stabilizeRival(v:VehicleTelemetry,control:VehicleControl):VehicleControl {
 ...
 const maxSteer=Math.min(.53/(1+Math.abs(v.speed)*.037),Math.atan(5.4*2.667/Math.max(v.speed*v.speed,1))),angle=Math.max(-.24,Math.min(.24,slip*.45-v.angularVelocity.y*.08));
