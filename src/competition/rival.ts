...
export class RivalController {
 ...
 constructor(route:CourseRoute,readonly id:string,readonly seed=1,readonly profileId:HandlingProfileId='legacy-p08a'){this.road=new RaceRoad(route);this.trait={...(traits[id]??traits.player)};if(profileId==='slingmods-sport-v1'){this.trait.pace*=2.28;this.trait.lateral*=route.width<13?1.4:1.8;this.trait.braking*=1.0;this.trait.headway*=1.15}let h=seed>>>0;for(const c of id)h=Math.imul(h^c.charCodeAt(0),16777619)>>>0;this.preference=this.trait.line+((h%1000)/1000-.5)*.25;this.targetLane=this.preference}
