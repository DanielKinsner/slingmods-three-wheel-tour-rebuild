...
export class RivalController {
 ...
 constructor(route:CourseRoute,readonly id:string,readonly seed=1){this.road=new RaceRoad(route);this.trait=traits[id]??traits.player;let h=seed>>>0;for(const c of id)h=Math.imul(h^c.charCodeAt(0),16777619)>>>0;this.preference=this.trait.line+((h%1000)/1000-.5)*.25;this.targetLane=this.preference}
 control(v:VehicleTelemetry,peers:Record<string,VehicleTelemetry>,dt=1/60):VehicleControl {
  ...
  const p=this.road.project(v.position.x,v.position.z,this.segment);this.segment=p.segment;const speed=Math.abs(v.speed),q=v.quaternion,yaw=Math.atan2(2*(q.w*q.y+q.x*q.z),1-2*(q.y*q.y+q.z*q.z)),lateral=(v.position.x-p.x)*-p.dz+(v.position.z-p.z)*p.dx;
  if(this.time>=this.nextPlan){this.stats.plans++;this.nextPlan=this.time+1/6;let nearest:{gap:number;speed:number;lane:number}|null=null;const traffic:{ahead:number;lane:number;speed:number}[]=[];
   ...
   this.targetSpeed=this.trait.pace;for(let d=0;d<=75;d+=5){const a=this.road.sample(p.progress+d-7),b=this.road.sample(p.progress+d+7),curvature=Math.abs(Math.atan2(a.dx*b.dz-a.dz*b.dx,a.dx*b.dx+a.dz*b.dz))/14,corner=Math.sqrt(this.trait.lateral/Math.max(curvature,.001));this.targetSpeed=Math.min(this.targetSpeed,Math.sqrt(corner*corner+2*this.trait.braking*Math.max(0,d-7)))}
   ...
   if(p.distance>3.9)this.targetSpeed=Math.min(this.targetSpeed,7);
  }
