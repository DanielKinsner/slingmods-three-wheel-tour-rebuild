...
export function createCrewGrid(route:CourseRoute,order:readonly string[]=CREW_IDS):Record<string,GridPose>{
 ...
 order.forEach((id,index)=>{const d=-6-Math.floor(index/2)*7-(index%2)*1.5,p=road.sample(d),lane=order.length===1?0:index%2===0?-2:2,yaw=Math.atan2(-p.dx,-p.dz),pose={x:p.x-p.dz*lane,y:route.start.y,z:p.z+p.dx*lane,yaw};grid[id]=pose;corners[id]=[];
