...
export type GridPose={x:number;y:number;z:number;yaw:number;pitch?:number};
...
export function createCrewGrid(route:CourseRoute,order:readonly string[]=CREW_IDS):Record<string,GridPose>{
 if(![1,2,4].includes(order.length)||new Set(order).size!==order.length)throw Error('Crew grid requires four unique participants');
 const road=new RaceRoad(route),grid:Record<string,GridPose>={},corners:Record<string,{x:number;z:number}[]>={};
 order.forEach((id,index)=>{const d=-6-Math.floor(index/2)*7-(index%2)*1.5,p=road.sample(d),lane=order.length===1?0:index%2===0?-2:2,yaw=Math.atan2(-p.dx,-p.dz),pose={x:p.x-p.dz*lane,y:p.y===undefined?route.start.y:p.y+.025,z:p.z+p.dx*lane,yaw,...(p.dy===undefined?{}:{pitch:Math.atan(p.dy)})};grid[id]=pose;corners[id]=[];
  for(const x of [-layout.width/2,layout.width/2])for(const z of [-layout.length/2,layout.length/2]){const c={x:pose.x+Math.cos(yaw)*x+Math.sin(yaw)*z,z:pose.z-Math.sin(yaw)*x+Math.cos(yaw)*z};corners[id].push(c);const g=route.checkpoints[0];if((c.x-g.x)*g.dx+(c.z-g.z)*g.dz>=-.5||road.project(c.x,c.z).distance>route.width/2-.2)throw Error(`Unsafe crew grid footprint: ${id}`);if(route.colliders.some(b=>pointInsideBox(b,c.x,pose.y+.4,c.z,.2)))throw Error(`Grid collider overlap: ${id}`)}
 });
