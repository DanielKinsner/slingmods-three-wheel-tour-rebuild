...
export async function loadExpressPresentation(scene:THREE.Scene,preset:'day'|'night'='day',renderer?:THREE.WebGLRenderer,lampsOn=preset==='night',wet=false){
 ...
 // P11 surface: true-scale race asphalt (ribbon UVs are 6 m per unit) and one merged decal layer with worn paint in place of
 // the flat white markings. Any failure here leaves the previous road exactly as it was.
 const surface=renderer?await Promise.all([loadRaceAsphalt(renderer,6,{wet}),loadRoadDecals(renderer,EXPRESS_ROUTE,{halfWidth:7.5,y:.016,lanePaint:{edge:7.36,centreDash:[6,18]},gridStation:0,wet})]).catch(error=>{console.warn('P11 road surface unavailable; keeping the previous road.',error);return undefined}):undefined;
 // After rain the painted kerbs and the concrete run-off shine too (visual only; the simulation's grip is untouched).
 if(wet)for(const m of[materials.runoff,materials.red,materials.white])m.roughness=.3;
 if(surface){road.material=surface[0].material;root.add(surface[1].mesh);flatPaint.forEach(m=>m.visible=false)}
 const trackside=renderer?await loadTrackside(renderer,planTrackside(EXPRESS_ROUTE,EXPRESS_TRACKSIDE),lampsOn).catch(error=>{console.warn('P11 trackside unavailable; keeping the plain rails.',error);return undefined}):undefined;if(trackside){root.add(trackside.group);rails.visible=false}
 const dashes:THREE.BufferGeometry[]=[];for(let d=12;d<EXPRESS_ROUTE.length;d+=18){const p=sampleRoad(EXPRESS_ROUTE,d),g=new THREE.PlaneGeometry(.13,7);g.rotateX(-Math.PI/2);g.rotateY(Math.atan2(p.dx,p.dz));g.translate(p.x,.026,p.z);dashes.push(g)}flatPaint.push(add(mergeGeometries(dashes)!,materials.white,'Express_center_dashes'));dashes.forEach(g=>g.dispose());
 ...
 // Fit the unit-width logo plane to the gantry header face: kit header is .75 m tall, centred 4.6 m up, front face +.3 m,
