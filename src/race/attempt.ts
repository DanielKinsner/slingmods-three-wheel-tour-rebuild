...
export function crossGate(g:CourseGate,a:{x:number;y?:number;z:number},b:{x:number;y?:number;z:number}){const da=(a.x-g.x)*g.dx+(a.z-g.z)*g.dz,db=(b.x-g.x)*g.dx+(b.z-g.z)*g.dz;if(!((da<0&&db>=0)||(da>0&&db<=0)))return null;const fraction=da/(da-db),x=a.x+(b.x-a.x)*fraction,z=a.z+(b.z-a.z)*fraction;if(g.y!==undefined&&(!Number.isFinite(a.y)||!Number.isFinite(b.y)||Math.abs(a.y!+(b.y!-a.y!)*fraction-g.y)>3))return null;if(Math.abs((x-g.x)*-g.dz+(z-g.z)*g.dx)>g.halfWidth)return null;return {fraction,forward:db>da}}
