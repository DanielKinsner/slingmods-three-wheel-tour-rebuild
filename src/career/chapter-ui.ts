...
export class ChapterUI {
 ...
 constructor(private client:CareerClient,private hooks:ChapterHooks){
  ...
  if(client.profile==='demo'){this.root.querySelector('.chapter-eyebrow')!.textContent='DEVELOPMENT PREVIEW / DEMO PROFILE';this.root.querySelector('h1')!.innerHTML='Your harbor.<br>Your build.';this.root.querySelector('#chapter-time-trial')!.textContent='Daylight Drive';this.root.querySelector('#chapter-build')!.textContent='Shop This Build'}this.unsubscribe=client.subscribe(()=>this.update());this.update();
 }
 ...
 update(){const s=this.client.state;this.root.setAttribute('aria-busy',String(this.pending));this.root.querySelector('#chapter-status')!.textContent=s.crew.cleared?'Chapter complete. Race the crew again, set a faster solo lap, or make the build yours.':s.chapters.firstCompletion?'Maya, Jett and Nico are waiting. Two laps after dark. Finish top three to join the crew.':'One clean lap earns your first credits. Then join three rivals for a night at the Harbor.';
  ...
  if(this.client.profile==='demo'){this.root.querySelector('#continue-chapter')!.textContent='Race the Harbor';this.root.querySelector('#chapter-status')!.textContent='A prepared demo profile: the SM-133 kit and crew event are available here. Your career stays separate.';this.root.querySelector('#chapter-wallet')!.textContent='Demo · '+s.credits+' game credits · '+(s.equipped?'Kit installed':'Stock appearance')+' · This tab only'}this.root.querySelector('#chapter-feedback')!.textContent=this.message;const invitation=this.root.querySelector<HTMLElement>('#crew-invitation')!;invitation.hidden=!this.invitation;
