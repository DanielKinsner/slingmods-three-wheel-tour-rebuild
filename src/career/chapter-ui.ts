...
export class ChapterUI {
 ...
 update(){const s=this.client.state,b=s.buildMatters,u=s.suspension,ryker=s.ownBuild.vehicle==='can-am-ryker-900',demo=this.client.profile==='demo';
  ...
  this.root.querySelector('#chapter-roadmap')!.innerHTML=`<li>${s.chapters.firstCompletion?'✓':'01'} Solo shakedown <small>First clean lap: 800 credits · repeats: 100</small></li><li>${b.duelCompleted?'✓':'02'} Maya / Find your line <small>One-rival duel · 1 lap · first finish: 650–750</small></li><li>${(ryker?s.ownBuild.ryker?.owned.includes('shocks'):u.owned)?'✓':'03'} Workshop / Build matters <small>Optional · ${ryker?'Elka Stage 3':'DDMWorks SM-3223'} · 1000 credits</small></li><li>${s.crew.cleared?'✓':'04'} First Night / Crew race <small>2-lap crew event · first podium: +400</small></li>`;
