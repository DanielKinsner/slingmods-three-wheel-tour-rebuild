...
export class ChapterUI {
 ...
 constructor(private client:CareerClient,private hooks:ChapterHooks){
  ...
  const nextChapter=document.createElement('a');nextChapter.href='?scene=career&play=career';nextChapter.textContent='Harbor Express / Chapter 02 →';nextChapter.className='chapter-next';nextChapter.style.cssText='display:inline-block;color:inherit;padding:12px 0;font-weight:bold';this.root.querySelector('.chapter-actions')!.append(nextChapter);
 ...
 update(){const s=this.client.state,b=s.buildMatters,u=s.suspension,demo=this.client.profile==='demo';
  ...
  this.root.querySelector('#chapter-roadmap')!.innerHTML=`<li>${s.chapters.firstCompletion?'✓':'01'} Solo shakedown <small>First clean lap: 800 credits · repeats: 100</small></li><li>${b.duelCompleted?'✓':'02'} Maya / Find your line <small>One-rival duel · 1 lap · first finish: 650–750</small></li><li>${u.owned?'✓':'03'} Workshop / Build matters <small>DDMWorks SM-3223 · 1000 credits</small></li><li>${s.crew.cleared?'✓':'04'} First Night / Crew race <small>2-lap crew event · first podium: +400</small></li>`;
