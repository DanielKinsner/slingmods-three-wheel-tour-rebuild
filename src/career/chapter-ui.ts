...
export class ChapterUI {
 readonly root=document.createElement('section');readonly menu:BuildMenuInput;private unsubscribe:()=>void;private pending=false;private invitation=false;
 constructor(private client:CareerClient,private hooks:ChapterHooks){
  ...
  this.root.innerHTML='<span class="chapter-eyebrow">SLINGMODS / CHAPTER 01</span><h1>Build Matters</h1><p id="chapter-status"></p><ol id="chapter-roadmap"></ol><div class="chapter-actions"><button id="continue-chapter" class="chapter-primary">Continue chapter</button><button id="chapter-time-trial">Time trial</button><button id="chapter-duel">Maya duel</button><button id="chapter-crew">Crew race</button><button id="chapter-build">Build</button></div><small id="chapter-wallet"></small><p id="chapter-feedback" role="status"></p><aside id="crew-invitation" hidden><strong>RAE / THE CREW</strong><p id="crew-lines"></p><button id="accept-crew">Skip dialogue · Race</button><button id="dismiss-crew">Later</button></aside>';
 ...
 private invite(){this.invitation=true;this.update();this.root.querySelector<HTMLButtonElement>('#accept-crew')!.focus()}
