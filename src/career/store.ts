...
export interface Receipt {handlingProfile?:'legacy-p08a'|'slingmods-sport-v1'|'slingmods-sport-v2'|'slingmods-sport-v3'|'slingmods-sport-v4'|'slingmods-sport-v5';id:string;kind:'award'|'purchase'|'crew-award'|'duel-award'|'suspension-purchase'|'chapter-award'|'chapter-purchase';amount:number;balance:number;first:boolean;at:string;event?:string;productId?:string;chapterRecord?:RaceRecord;place?:number;timeMs?:number;chapterBonus?:number}
...
export function transition(current:Career,command:Command,at=new Date().toISOString()):Result {
 if('handlingProfile' in command&&command.handlingProfile!==undefined&&!['legacy-p08a','slingmods-sport-v1','slingmods-sport-v2','slingmods-sport-v3','slingmods-sport-v4','slingmods-sport-v5'].includes(command.handlingProfile))throw Error('Unknown handling profile');
 ...
 if(receipt&&'handlingProfile' in command){if(!['legacy-p08a','slingmods-sport-v1','slingmods-sport-v2','slingmods-sport-v3','slingmods-sport-v4','slingmods-sport-v5'].includes(command.handlingProfile!))throw Error('Unknown handling profile');receipt.handlingProfile=command.handlingProfile;}
...
export function migrateCareer(raw:unknown):Career {
 ...
 for(const [id,r]of Object.entries(raw.receipts))if(!object(r)||r.id!==id||(r.handlingProfile!==undefined&&!['legacy-p08a','slingmods-sport-v1','slingmods-sport-v2','slingmods-sport-v3','slingmods-sport-v4','slingmods-sport-v5'].includes(r.handlingProfile))||!uuid.test(id)||!['award','purchase',...(raw.version>=2?['crew-award']:[]),...(raw.version>=3?['duel-award','suspension-purchase']:[]),...(raw.version===4?['chapter-award','chapter-purchase']:[])].includes(r.kind)||!Number.isSafeInteger(r.amount)||!Number.isSafeInteger(r.balance)||r.balance<0||typeof r.first!=='boolean'||typeof r.at!=='string'||!Number.isFinite(Date.parse(r.at)))return fail();
