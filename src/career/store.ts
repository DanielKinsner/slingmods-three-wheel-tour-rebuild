...
export interface Receipt {id:string;kind:'award'|'purchase'|'crew-award';amount:number;balance:number;first:boolean;at:string;event?:typeof CREW_EVENT;place?:number;timeMs?:number;chapterBonus?:number}
export interface Career {version:2;revision:number;credits:number;owned:boolean;equipped:boolean;appearance:Appearance;chapters:{entry:boolean;firstCompletion:boolean;firstBuild:boolean};crew:{invitationSeen:boolean;completed:boolean;bestPlace:number|null;cleared:boolean;clearAcknowledged:boolean};receipts:Record<string,Receipt>}
export const freshCareer=():Career=>({version:2,revision:0,credits:0,owned:false,equipped:false,appearance:defaultAppearance(),chapters:{entry:false,firstCompletion:false,firstBuild:false},crew:{invitationSeen:false,completed:false,bestPlace:null,cleared:false,clearAcknowledged:false},receipts:{}});
export type Command={type:'award';id:string;valid:boolean;timeMs:number;event:'harbor'}|{type:'purchase';id:string;productId:string;vehicleId:string}|{type:'equip';equipped:boolean}|{type:'appearance';patch:Partial<Appearance>}|{type:'entry'}|{type:'crew-award';result:CrewAwardProof}|{type:'crew-invitation'}|{type:'crew-acknowledge'};
export interface Result {state:Career;changed:boolean;receipt?:Receipt;story?:'entry'|'firstCompletion'|'firstBuild'|'crewPodium'|'crewWin'|'crewFourth'}
...
export function migrateCareer(raw:unknown):Career {
 ...
 if(!object(raw)||![1,2].includes(raw.version))return fail();
 ...
 for(const [id,r]of Object.entries(raw.receipts))if(!object(r)||r.id!==id||!uuid.test(id)||!['award','purchase',...(raw.version===2?['crew-award']:[])].includes(r.kind)||!Number.isSafeInteger(r.amount)||!Number.isSafeInteger(r.balance)||r.balance<0||typeof r.first!=='boolean'||typeof r.at!=='string'||!Number.isFinite(Date.parse(r.at)))return fail();
...
class IndexedCareerStore implements CareerStore {
 ...
 read():Promise<Career>{return new Promise((resolve,reject)=>{
  ...
  request.onsuccess=()=>{try{s=request.result===undefined?freshCareer():migrateCareer(request.result);if(request.result===undefined||request.result.version===1)store.put(s,'current')}catch(e){error=e;tx.abort()}};
 ...
 execute(command:Command):Promise<Result>{return new Promise((resolve,reject)=>{
  ...
  request.onsuccess=()=>{try{result=transition(request.result===undefined?freshCareer():migrateCareer(request.result),command);if(result.changed||request.result?.version===1)store.put(result.state,'current')}catch(e){error=e;tx.abort()}};
