import {VEHICLE_VISUAL} from '../presentation/vehicle-asset';
...
export async function careerClient(){
 ...
 const query=new URLSearchParams(location.search);if(['bay','career'].includes(query.get('scene')??'')){const visual=query.get('visual');if(visual==='ryker'||visual==='2026'){const result=await store.execute({type:'chapter-vehicle',vehicle:visual==='ryker'?'can-am-ryker-900':'slingshot-r-2024'});state=result.state}else if((VEHICLE_VISUAL==='ryker')!==(state.ownBuild.vehicle==='can-am-ryker-900')){const target=new URL(location.href);target.searchParams.set('visual',state.ownBuild.vehicle==='can-am-ryker-900'?'ryker':'2026');if(!store.durable)writeTransfer(target.href,profile,state,visitor);location.replace(target.href);await new Promise<never>(()=>{})}}
