...
export async function careerClient(){
 ...
 try{if(window.name.startsWith(prefix)){const transfer=JSON.parse(window.name.slice(prefix.length));window.name=typeof transfer.previousName==='string'?transfer.previousName:'';if(transfer.origin===location.origin&&transfer.target===location.pathname+location.search&&[1,2].includes(transfer.state?.version))fallback=migrateCareer(transfer.state)}}catch{window.name=''}
 let store:CareerStore;try{store=await openCareer(fallback)}catch(e){if(!(e instanceof CareerDataError))throw e;store=await recoveryChoice(e)};let state=await store.read(),closed=false,stale=false;const listeners=new Set<()=>void>();
 ...
 function handoff(url:string){if(store.durable)return;const target=new URL(url,location.href);if(target.origin!==location.origin||!['bay','pad','vehicle','harbor','crew'].includes(target.searchParams.get('scene')??'bay'))return;window.name=prefix+JSON.stringify({origin:location.origin,target:target.pathname+target.search,previousName:window.name,state})}
