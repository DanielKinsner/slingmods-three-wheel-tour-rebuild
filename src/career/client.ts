...
export async function careerClient(){
 ...
 function handoff(url:string){if(store.durable)return;const target=new URL(url,location.href);if(target.origin!==location.origin||profileFrom(target.search)!==profile||!['bay','pad','vehicle','harbor','crew'].includes(target.searchParams.get('scene')??'bay'))return;window.name=prefix+JSON.stringify({profile,origin:location.origin,target:transferTarget(target,import.meta.env.MODE==='demo'),previousName:window.name,state})}
