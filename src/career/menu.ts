...
export class BuildMenuInput {
 private armed=false;private padKey='';private prior=[false,false,false,false];
 constructor(private root:HTMLElement,private back:()=>void){}
 reset(){this.armed=false;this.padKey='';this.prior=[false,false,false,false]}
 ...
 move(direction:number){const choices=[...this.root.querySelectorAll<HTMLElement>('button:not(:disabled),a[href]:not([aria-disabled=true]),input:not(:disabled),summary')].filter(e=>!e.hidden&&!!e.getClientRects().length);if(!choices.length)return;const current=choices.indexOf(document.activeElement as HTMLElement);choices[(current+direction+choices.length)%choices.length].focus()}
}
