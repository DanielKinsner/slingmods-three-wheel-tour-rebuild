...
export function rivalDecal(source:THREE.Texture,color:string){
 ...
 c.putImageData(data,0,0);const map=new THREE.CanvasTexture(canvas);map.colorSpace=source.colorSpace;map.flipY=source.flipY;map.channel=source.channel;map.anisotropy=source.anisotropy;return map;
}
