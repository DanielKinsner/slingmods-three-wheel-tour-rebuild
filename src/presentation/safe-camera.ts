...
export class ResultCamera {
 ...
 constructor(private camera:THREE.PerspectiveCamera,private heightAt:(x:number,z:number)=>number=()=>0,private parkedContact=true){}
 update(root:THREE.Object3D,show:boolean,width:number,height:number){
  ...
  this.active=true;if(this.parkedContact){if(!this.contact){this.contact=vehicleContact();root.add(this.contact.mesh)}this.contact.mesh.visible=true;this.contact.ground(root,this.heightAt)}root.updateWorldMatrix(true,true);const direction=new THREE.Vector3(-5,1.65,-6).applyQuaternion(root.quaternion);
