...
export async function loadHarborSpeedDressing(renderer:THREE.WebGLRenderer,scene:THREE.Scene,route:CourseRoute,night:boolean,wet=false){
 ...
 const [asphalt,decals,trackside]=await Promise.all([roads.length?loadRaceAsphalt(renderer,metresPerUV(roads[0].geometry),{vertexColors:!!roads[0].geometry.getAttribute('color'),wet}):undefined,loadRoadDecals(renderer,route,{halfWidth:route.width/2,y:.008,lanePaint:null,gridStation:0,seed:2207,wet}),loadTrackside(renderer,planTrackside(route,fenced),night)]);
 ...
 return{group,roadY:0,wetSurfaces:asphalt?.wet?[asphalt.wet]:[],reflectable:[trackside.group]as THREE.Object3D[],update:(camera:THREE.Vector3)=>trackside.update(camera),inspect:()=>({roadMeshes:roads.length,asphalt:asphalt?.inspect()??null,decals:decals.inspect(),trackside:trackside.inspect()}),
