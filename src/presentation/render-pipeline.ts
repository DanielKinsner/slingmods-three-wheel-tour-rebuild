...
export const DEFAULT_BLOOM={threshold:1.6,intensity:.32},NIGHT_BLOOM={threshold:1.05,intensity:.5},
/** Showroom: lamps are inspected from a metre away under bright studio light, so they get a halo, not a flare. */
SHOWROOM_BLOOM={threshold:2.8,intensity:.14};
/** Each blurrier mip adds this fraction of the previous one: a tight halo around a lamp, not a haze across the frame. */
const BLOOM_SCATTER=.7,
/** Intensities are expressed against five full-strength levels (the High preset before scatter), so every preset glows alike. */
BLOOM_REFERENCE_LEVELS=5;
export class RenderPipeline {
 ...
 constructor(private renderer:THREE.WebGLRenderer,quality:GraphicsQuality){
  ...
  // Peaks are capped (hue kept) at 6x white: a one-pixel specular glint on a bolt must not bloom like a lamp (fireflies).
  this.prefilter=material(`uniform sampler2D tSource;uniform vec2 uvScale;uniform float threshold;varying vec2 vUv;
void main(){vec3 c=texture2D(tSource,min(vUv,vec2(.999))*uvScale).rgb;float peak=max(c.r,max(c.g,c.b));c*=min(1.,6./max(peak,1e-4));float l=max(c.r,max(c.g,c.b)),knee=threshold*.5,soft=clamp(l-threshold+knee,0.,2.*knee);soft=soft*soft/(4.*knee+1e-4);gl_FragColor=vec4(c*max(soft,l-threshold)/max(l,1e-4),1.);}`,{tSource:{value:null},uvScale:{value:new THREE.Vector2(1,1)},threshold:{value:DEFAULT_BLOOM.threshold}});
...
void main(){vec3 c=texture2D(tSource,vUv).rgb*4.;c+=texture2D(tSource,vUv+texel*vec2(-1.,-1.)).rgb+texture2D(tSource,vUv+texel*vec2(1.,-1.)).rgb+texture2D(tSource,vUv+texel*vec2(-1.,1.)).rgb+texture2D(tSource,vUv+texel*vec2(1.,1.)).rgb;gl_FragColor=vec4(c/8.,1.);}`,{tSource:{value:null},texel:{value:new THREE.Vector2()}});
  this.up=material(`uniform sampler2D tSource;uniform vec2 texel;uniform float scatter;varying vec2 vUv;
void main(){vec3 c=texture2D(tSource,vUv).rgb*4.;c+=(texture2D(tSource,vUv+texel*vec2(-1.,0.)).rgb+texture2D(tSource,vUv+texel*vec2(1.,0.)).rgb+texture2D(tSource,vUv+texel*vec2(0.,-1.)).rgb+texture2D(tSource,vUv+texel*vec2(0.,1.)).rgb)*2.;c+=texture2D(tSource,vUv+texel*vec2(-1.,-1.)).rgb+texture2D(tSource,vUv+texel*vec2(1.,-1.)).rgb+texture2D(tSource,vUv+texel*vec2(-1.,1.)).rgb+texture2D(tSource,vUv+texel*vec2(1.,1.)).rgb;gl_FragColor=vec4(c/16.*scatter,1.);}`,{tSource:{value:null},texel:{value:new THREE.Vector2()},scatter:{value:BLOOM_SCATTER}},THREE.AdditiveBlending);
...
void main(){
 ...
 render(scene:THREE.Scene,camera:THREE.PerspectiveCamera,look:FrameLook={},now=performance.now()){
  ...
  if(this.bloomTargets.length&&bloom.intensity>0){
   ...
   let energy=0;for(let i=0;i<chain.length;i++)energy+=BLOOM_SCATTER**i;uv.tBloom.value=chain[0].texture;uv.bloomIntensity.value=bloom.intensity*BLOOM_REFERENCE_LEVELS/energy;
