...
export async function loadShowcaseHarbor(scene:THREE.Scene,legacyHarbor:THREE.Object3D,preset:'day'|'night',stage:'sample'|'full'='full'){
 ...
 const root=new THREE.Group();root.name='P06B_Harbor_Quality';scene.add(root);root.add(foundation.scene);
 ...
 for(const [key,placements]of batches){
  ...
  for(const [level,variant]of variants.entries())for(const piece of library.get(variant)??[]){
   ...
   mesh.receiveShadow=placements[0].module!=='water';mesh.castShadow=(['terminal','warehouse','pavilion','gantry'].includes(placements[0].module)||placements[0].module.startsWith('palm'))&&level===0;mesh.frustumCulled=true;
 ...
 const water=createHarborWater(waterMaterials);
 ...
 const materialBindings=[...actualMaterials].map(m=>{const p=m as THREE.MeshStandardMaterial;return{name:m.name,packedImageBindings:m.userData.p06bImageBindings,materialSource:m.userData.p06bMaterialBinding,basecolor:p.map?.name??null,normal:p.normalMap?.name??null,roughness:p.roughnessMap?.name??null,occlusion:p.aoMap?.name??null,normalScale:p.normalScale?.toArray(),colorSpace:p.map?.colorSpace,side:m.side}});
 ...
 return{
  ...
  inspect:()=>({version:layout.version,stage,materialBindings,routeSHA256:layout.routeSHA256,districts:layout.districts,placementCount:use.length,instanceGroups:instances.length,foundationTriangles,instancedTrianglesAllPlacements,totalAuthoredSceneTriangles:foundationTriangles+instancedTrianglesAllPlacements,triangleCensusMethod:'All placed source triangles including instance multiplicity; actual culled and shadow-pass submission measured separately with renderer.info',spatialChunkMetres:layout.spatialChunkMetres,culledBy:'per-cell InstancedMesh computed sphere and frustum; no whole-harbor instance bounds',geometryLOD:{method:'Authored near/mid/far palm geometry, same root and crown envelopes, selected per spatial cell at nearest bound distance',thresholdsMetres:layout.lod?.distancesMetres??[],preparation:'All LOD buffers and material variants prepared behind loading veil'},groups:instances.map(m=>({name:m.name,count:m.count,bounds:m.boundingSphere?{center:m.boundingSphere.center.toArray(),radius:m.boundingSphere.radius}:null,trianglesPerInstance:(m.geometry.index?.count??m.geometry.attributes.position.count)/3,castsShadow:m.castShadow,lod:m.userData.lodLevel,visible:m.visible})),cameraDistance,water:water.inspect(),source:'assets/blender/showcase-quality/quality-kit.blend',foundation:'Presentation-only replacement; protected physical route/colliders recorded separately in P06B/preservation-final.json'}),
