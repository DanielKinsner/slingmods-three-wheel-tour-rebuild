...
import * as THREE from 'three';import type{GLTFLoader}from'three/addons/loaders/GLTFLoader.js';import{COLORS,defaultAppearance,type Appearance}from'../career/catalog';
...
export class ProductPresenter {
 ...
 private constructor(readonly root:THREE.Group,private chassis:THREE.Object3D,private scene:THREE.Scene,readonly descriptor:any){
  ...
  for(const origin of origins.slice(0,2)){this.origins.push(new THREE.Vector3().fromArray(origin));const light=new THREE.RectAreaLight(0xffffff,0,.035,1.3);light.castShadow=false;this.lights.push(light)}
 }
 static async load(loader:GLTFLoader,chassis:THREE.Object3D,scene:THREE.Scene){const [g,descriptor]=await Promise.all([loader.loadAsync('/assets/products/tricled-sm133-base.glb'),fetch('/assets/products/tricled-sm133-base.attachment.json').then(r=>{if(!r.ok)throw Error('Accessory attachment unavailable');return r.json()})]);return new ProductPresenter(g.scene,chassis,scene,descriptor)}
 set(equipped:boolean,appearance:Appearance){if(this.disposed)return;this.equipped=equipped;this.appearance={...appearance};if(equipped){if(!this.root.parent)this.chassis.add(this.root)}else this.root.removeFromParent();const lit=equipped&&appearance.enabled;
  ...
  this.lights.forEach(l=>{if(lit){if(!l.parent)this.scene.add(l)}else{l.removeFromParent()}l.color.set(COLORS[appearance.color]);l.intensity=lit?appearance.brightness*14:0});this.update();
 }
 ...
 inspect(){let triangles=0,meshes=0;this.root.traverse(o=>{if(o instanceof THREE.Mesh){meshes++;triangles+=(o.geometry.index?.count??o.geometry.attributes.position.count)/3}});return {equipped:this.equipped,appearance:{...this.appearance},attached:!!this.root.parent,lights:this.lights.filter(l=>!!l.parent).length,extraShadowMaps:0,meshes,triangles,lightOrigins:this.origins.map(v=>v.toArray()),method:'Two downward narrow rectangular area lights aligned with chassis-mounted side strips, plus emissive Blender diffusers. Actual receiver geometry/normals and distance falloff; no glow decal or new shadow maps. Artistic approximation: no fine underside/barrier occluder shadows, not measured photometry.'}}
