...
function selectedVisual(){
 ...
 try{const raw=new URLSearchParams(location.hash.slice(1)).get('build');const recipe=raw?JSON.parse(raw):null;if(recipe?.version===2&&recipe.vehicleId==='can-am-ryker-900')requested='ryker';else if(recipe?.version===1&&recipe.vehicleId==='slingshot-r-2024'&&!recipe.ryker)requested='2026'}catch{/* Recipe validation reports malformed links. */}
 const valid=(v:string|null)=>v==='josh'||v==='legacy'||v==='2026'||v==='ryker';
...
export const CURRENT_VEHICLE=VEHICLE_VISUAL==='ryker'?'ryker-900-complete.glb':VEHICLE_VISUAL==='2026'?'slingshot-2026.glb':VEHICLE_VISUAL==='josh'?'slingshot-josh-adapted.glb':'slingshot-hoops-refined.glb';
export const CURRENT_REAR_RIG=VEHICLE_VISUAL==='ryker'?'/assets/ryker/rear-rig.json':VEHICLE_VISUAL==='2026'?'/assets/model02/rear-rig.json':'/assets/vehicles/slingshot-p04a1-rear-rig.json';
export const CURRENT_DRIVER_ATTACHMENT=VEHICLE_VISUAL==='ryker'?'/assets/ryker/driver-attachment.json':VEHICLE_VISUAL==='2026'?'/assets/model02/driver-attachment.json':'/assets/drivers/test-driver-attachment.json';
...
export const CURRENT_VEHICLE_URL=VEHICLE_VISUAL==='ryker'?'/assets/ryker/complete/ryker-900-complete.glb':VEHICLE_VISUAL==='2026'?MODEL_2026_URL:VEHICLE_VISUAL==='josh'?JOSH_VEHICLE_URL:LEGACY_VEHICLE_URL;
