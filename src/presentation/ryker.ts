import {RykerLinkage} from './ryker-linkage';
...
export class RykerMotion {
 ...
 pose(t:VehicleTelemetry){
  this.wheels.forEach((b,i)=>{const w=t.wheels[i];b.carrier.position.copy(b.rest);b.carrier.position.y+=w.localCenter.y-layout.wheels[i].center[1];b.carrier.rotation.y=i<2?w.steer:0;
   ...
   b.spin.rotation.x=-w.spin;
  });this.handle.quaternion.copy(this.base).multiply(new THREE.Quaternion().setFromAxisAngle(this.axis,t.steer));
  this.linkage.pose(t);this.report={linkage:this.linkage.inspect(),kind:'ryker-rigid',wheelbase:1.709,physicsWheelbase:layout.wheelbase,physicsContactMismatch:false,handlebarAngle:t.steer,calipersSpin:false,fendersSpin:false,rearSuspension:'Articulated mono swingarm with independent coilover'};
 }
