...
export class RykerProducts {
 ...
 constructor(private car:THREE.Object3D,readonly root:THREE.Group){
  ...
  car.add(root);root.traverse(o=>{if(o instanceof THREE.Mesh){o.castShadow=true;o.receiveShadow=true}});this.set({});
 }
 set(state:RykerBuild={}){this.state={...state};for(const id of ['shocks','exhaust','body'] as const){const part=this.root.getObjectByName('ryker_mod_'+id);if(part)part.visible=!!state[id];for(const stock of ['stock_ryker_'+id,'stock_ryker_'+id+'_rear']){const node=this.root.getObjectByName(stock);if(node)node.visible=!state[id]}}const grille=this.car.getObjectByName('grille');if(grille)grille.visible=!state.body;}
 inspect(){return {selected:{...this.state},replacementNodes:this.hidden.map(o=>o.name),geometry:'Original reference-based game approximations; not manufacturer CAD',physics:'Unchanged shared Sport v5'}}
