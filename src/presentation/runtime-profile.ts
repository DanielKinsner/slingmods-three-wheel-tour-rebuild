...
export class RuntimeProfile {
 ...
 frame(now:number,interval:number,cpu:number,ticks:number,race:{phase:string;attempt:number;elapsedMs:number},camera:string,renderer:THREE.WebGLRenderer,scene:THREE.Scene,resolutionScale=1){
