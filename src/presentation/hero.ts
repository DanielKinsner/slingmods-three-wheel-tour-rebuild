...
export async function loadDrivingHero(loader:GLTFLoader){
 ...
 return{root,asset:asset.scene,driver,rear,attachment,statistics:{car:assetStatistics(asset.scene),driver:assetStatistics(person.scene)},pose(t:VehicleTelemetry,dt:number,cockpit:boolean,reset=false){
