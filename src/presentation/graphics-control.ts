...
export function installGraphicsControl(parent:Element,renderer:THREE.WebGLRenderer,pipeline:RenderPipeline,loadedLighting:'low'|'standard'){
 const style=document.createElement('style');style.textContent=STYLE;document.head.append(style);
 const root=document.createElement('label'),select=document.createElement('select'),note=document.createElement('small');root.id='game-graphics';root.append('Graphics',select,note);select.setAttribute('aria-label','Graphics quality');
