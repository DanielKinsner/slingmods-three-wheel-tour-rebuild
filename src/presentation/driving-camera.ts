...
export class DrivingCamera {
 ...
 eye=new THREE.Vector3(-.43,1.12,.48);activeView:DrivingView|'rearward'='near';
 ...
 update(p:THREE.Vector3,q:THREE.Quaternion,speed:number,dt:number,mode:DrivingView,lookBack:boolean,snap=false,obstruction?:(target:THREE.Vector3,desired:THREE.Vector3)=>number|undefined){
  ...
  const forward=new THREE.Vector3(0,-.43,-1).normalize().applyQuaternion(orientation);if(reset)this.cockpitDirection.copy(forward);else this.cockpitDirection.lerp(forward,1-Math.exp(-24*Math.max(0,dt))).normalize();
  ...
  }else if(mode==='cockpit'){this.camera.position.copy(eye);this.target.copy(eye).addScaledVector(this.cockpitDirection,12);this.camera.fov=66;this.activeView='cockpit'}
