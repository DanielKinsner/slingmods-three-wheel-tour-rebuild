...
export function vehicleSelection(){
 ...
 return `<nav class="vehicle-appearance-picker" aria-label="Choose your vehicle"><span>YOUR RIDE</span>${[['2026','Slingshot R'],['ryker','Can-Am Ryker 900'],['spyder','Spyder F3 Custom']].map(([id,label])=>`<a href="${link(id).replace(/&/g,'&amp;')}" data-vehicle="${id}" aria-current="${(VEHICLE_VISUAL==='spyder'?'spyder':VEHICLE_VISUAL==='ryker'?'ryker':'2026')===id?'true':'false'}">${label}</a>`).join('')}</nav>`;
}
