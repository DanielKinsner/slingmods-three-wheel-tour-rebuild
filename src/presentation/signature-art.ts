...
export class SignatureProducts {
 ...
 inspect(){return{ryker:this.ryker?.inspect(),selected:[...this.selected],storageInspection:this.saved.size>0,stockExhaust:{meshes:this.stockMeshes,visible:this.stock?.visible,kind:this.stockMeshes?'geometry':'empty compatibility anchor'},seats:['driver','passenger'].map(side=>({side,visible:this.car.getObjectByName('signature_seat_'+side)?.visible})),inspectionMethod:'Only seats temporarily removed; original doors hinge open. Complete car visibility restored on exit.',mounts:SIGNATURE_MOUNT_VIEWS,productRoot:this.root.name}}
 dispose(){if(this.ryker){this.ryker.dispose();disposeSignatureObject(this.ryker.root)}this.inspectStorage(false);if(this.stock)this.stock.visible=this.stockVisible;this.root.removeFromParent();disposeSignatureObject(this.root)}
}
