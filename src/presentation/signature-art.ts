...
export class SignatureProducts {
 ...
 static async load(loader:GLTFLoader,car:THREE.Object3D){if(isRyker(car)){const gltf=await loader.loadAsync('/assets/ryker/ryker-accessories.glb');const result=new SignatureProducts(car,new THREE.Group());result.ryker=new RykerProducts(car,gltf.scene);return result}const gltf=await loader.loadAsync(car.getObjectByName('model02_2026_foundation')?'/assets/model02/2026-mounted-products.glb':car.getObjectByName('josh_donor_foundation')?'/assets/model01/josh-mounted-products.glb':SIGNATURE_PRODUCTS_URL);return new SignatureProducts(car,gltf.scene)}
