...
export interface GraphicsPreset {label:string;/** Render through the post pipeline. Every preset does; `false` exists for the automatic fallback when a GPU cannot. */post:boolean;/** MSAA samples on the scene target. */samples:number;pixelRatioCap:number;/** Bloom mip levels; 0 disables bloom. */bloomLevels:number;dynamicResolution:boolean;/** Existing load-time lighting/shadow budget. */lighting:'low'|'standard'}
export const GRAPHICS_PRESETS:Record<GraphicsQuality,GraphicsPreset>={
 ...
 low:{label:'Low',post:true,samples:0,pixelRatioCap:1,bloomLevels:0,dynamicResolution:true,lighting:'low'},
 medium:{label:'Medium',post:true,samples:2,pixelRatioCap:1,bloomLevels:4,dynamicResolution:true,lighting:'standard'},
 high:{label:'High',post:true,samples:4,pixelRatioCap:1.5,bloomLevels:5,dynamicResolution:true,lighting:'standard'},
 ultra:{label:'Ultra',post:true,samples:4,pixelRatioCap:2,bloomLevels:6,dynamicResolution:false,lighting:'standard'}};
