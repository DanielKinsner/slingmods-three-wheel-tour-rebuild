...
export class VehicleEffects {
 ...
 private cars:{spec:EffectCar;ryker:boolean;spyder:boolean;state:VehicleEffectState;lastTime:number;lastSpeed:number;anchors:THREE.Vector3[];marking:boolean[];budget:Float32Array;contactBudget:number;outlets:THREE.Vector3[]}[];
 ...
 constructor(scene:THREE.Scene,cars:EffectCar[],private readContact:ContactReader=()=>false,readonly enabled=true){
  ...
  // Hollow beam cone: fade where it is seen edge-on and where its lower half would slice into the road, so it reads as a soft
  // shaft of light instead of hard white lines (thin additive surfaces pile up at grazing angles).
  const cone=new THREE.ConeGeometry(1.2,9,16,1,true);cone.translate(0,-4.5,0);cone.rotateX(Math.PI/2);
  this.cones=new THREE.InstancedMesh(cone,new THREE.ShaderMaterial({vertexShader:'varying vec3 p;varying vec3 vn;varying vec3 vv;void main(){p=position;mat4 mv=modelViewMatrix*instanceMatrix;vec4 v=mv*vec4(position,1.);vv=v.xyz;vn=mat3(mv)*normal;gl_Position=projectionMatrix*v;}',fragmentShader:'varying vec3 p;varying vec3 vn;varying vec3 vv;void main(){float d=clamp(-p.z/9.,0.,1.);float facing=abs(dot(normalize(vn),normalize(-vv)));float a=.016*smoothstep(0.,.12,d)*(1.-d)*(1.-d)*facing*facing*smoothstep(-.45,-.15,p.y);gl_FragColor=vec4(.68,.79,1.,a);}',transparent:true,depthWrite:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending}),cars.length*2);this.cones.name='pooled-headlight-fog';this.cones.frustumCulled=false;this.group.add(this.cones);
 }
 ...
 update(field:Record<string,VehicleTelemetry>,camera:THREE.PerspectiveCamera,pixels:number,quality:GraphicsQuality,wet:boolean,lamps:boolean,reduced:boolean,reset=false){
  ...
  for(let ci=0;ci<this.cars.length;ci++){
   ...
   for(let wi=0;wi<3;wi++){
    ...
    visualWheel(this.p,wi,w.localCenter,ryker);this.p.y-=c.spyder?.32355:ryker?rykerWheels[wi].radius:(wi===2?.3455:.32985);this.p.applyQuaternion(this.q).add(t.position);
    ...
    if(dt>0&&skid){if(c.marking[wi]){const distance=c.anchors[wi].distanceTo(this.p);if(this.skids.add(c.anchors[wi],this.p,c.spyder?(wi===2?.273:.204):ryker?(wi===2?.168:.135):(wi===2?.27:.19))||distance>2.5)c.anchors[wi].copy(this.p)}else c.anchors[wi].copy(this.p)}c.marking[wi]=skid&&dt>0;
