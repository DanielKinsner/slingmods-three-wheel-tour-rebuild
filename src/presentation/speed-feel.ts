...
/**
 * Speed-reactive camera layer applied AFTER DrivingCamera. It only re-poses the presentation camera: no physics, input,
 ...
 */
export const SPEED_FEEL={topSpeedMph:120,restFov:55,topFov:78,cockpitFov:[66,75],pullBack:1.0,brakeTuck:.8,drop:.75,lookToVelocity:.6,yawLead:.22,rollPerG:1.25,maxRoll:2,blurFromMph:70,shakeFromMph:90,shakePixels:[.5,1.5],roughShakePixels:3.2} as const;
...
export class SpeedFeel {
 ...
 apply(camera:THREE.PerspectiveCamera,target:THREE.Vector3,t:VehicleTelemetry,view:DrivingView|'rearward',dt:number,viewportHeight:number,reset=false,obstruction?:(target:THREE.Vector3,desired:THREE.Vector3)=>number|undefined){
  ...
  const cockpit=view==='cockpit',fov=cockpit?THREE.MathUtils.lerp(SPEED_FEEL.cockpitFov[0],SPEED_FEEL.cockpitFov[1],s):THREE.MathUtils.lerp(SPEED_FEEL.restFov,SPEED_FEEL.topFov,Math.pow(s,.85))+Math.max(0,this.drive)*2*s;
  const reach=cockpit?0:s*(.55+.95*Math.max(0,this.drive))*SPEED_FEEL.pullBack-SPEED_FEEL.brakeTuck*Math.max(0,-this.drive)*Math.min(1,s*2.2),drop=cockpit?0:SPEED_FEEL.drop*s,roll=THREE.MathUtils.clamp(lateralG*SPEED_FEEL.rollPerG,-SPEED_FEEL.maxRoll,SPEED_FEEL.maxRoll)*(cockpit?-.6:1);
  ...
  if(cockpit){
   ...
   this.offset.copy(camera.position).sub(target);const length=this.offset.length(),desired=this.offset.multiplyScalar((length+this.reach)/Math.max(length,1e-6)).add(target);desired.y=Math.max(target.y-.2,desired.y-this.drop);
   const hit=this.reach>0?obstruction?.(target,desired):undefined;if(hit!==undefined&&hit<desired.distanceTo(target))desired.sub(target).setLength(Math.max(.5,hit-.3)).add(target);
