...
export class RaceFX {
 ...
 /** A rivalry call-out to use at the start instead of a stock line (race host, game/rivalry.ts). */
 startLine?:()=>{who:CrewId;text:string}|null;
 radio(moment:RadioMoment,who?:CrewId,force=false,text?:string){const now=performance.now();if(!force&&now-this.radioAt<4200)return;const stock=radioLine(moment,who),line=text&&who?{member:CREW[who],text}:stock;if(!line)return;this.radioAt=now;const m=line.member;this.radioNode.style.setProperty('--gx-crew',m.color);this.radioNode.innerHTML=`<img src="${m.portrait}" alt=""><div><b>${m.name}</b><span>${line.text}</span></div><i aria-hidden="true"></i>`;this.radioNode.classList.remove('is-on');void this.radioNode.offsetWidth;this.radioNode.classList.add('is-on');gameCue('gx.tab');clearTimeout(this.radioTimer);this.radioTimer=window.setTimeout(()=>this.radioNode.classList.remove('is-on'),3600)}
 ...
 update(s:RaceSnapshotLike,speed=0){
  ...
  if(s.phase==='countdown'&&!s.paused){const n=Math.ceil(s.countdown);if(this.phase!=='countdown'&&!this.freeDrive)setTimeout(()=>{const r=this.trial?null:this.startLine?.();if(r)this.radio('start',r.who,false,r.text);else this.radio(this.trial?'trialStart':'start')},350);if(n!==this.count){this.count=n;this.lights.classList.add('is-on');this.lights.dataset.lit=String(Math.max(0,3-n+1));this.lights.classList.remove('is-go');if(n>0){this.lights.querySelector('b')!.textContent=String(n);this.pulse(this.lights.querySelector('b')!)}}}
