...
export class ChallengeRun {
 private judge:ChallengeJudge;private hud=document.createElement('section');private result=document.createElement('section');private markers=new THREE.Group();private last:JudgeState['phase']='ready';private shown=false;
 constructor(parent:HTMLElement,scene:THREE.Scene,private route:CourseRoute,readonly challenge:Challenge,private vehicle:string,private nav:{retry:()=>void;next:(c:Challenge)=>void;menu:()=>void}){
  this.judge=new ChallengeJudge(route,challenge);
 ...
 private command(a:string){gameCue(a==='menu'?'gx.back':'gx.select');if(a==='retry'){this.hideResult();this.judge.reset();this.last='ready';this.nav.retry()}else if(a==='next')this.nav.next(this.nextChallenge());else if(a==='menu')this.nav.menu()}
 ...
 frame(t:VehicleTelemetry){
  ...
  if(s.phase==='ready'){q('value').textContent=c.kind==='sprint'?'0.000 s':c.kind==='trap'?formatValue(c,0,u):`${Math.round(this.judge.length)} m`;q('sub').textContent='Timer starts when you move';return}
  const toGo=Math.max(0,s.toGo);
  if(c.kind==='sprint'){q('value').textContent=formatValue(c,s.elapsedMs);q('sub').textContent=`${Math.round(toGo)} m to the gate`}
 ...
 dispose(){removeEventListener('keydown',this.onKey,true);this.hud.remove();this.result.remove();this.markers.removeFromParent();this.markers.traverse(o=>{if(o instanceof THREE.Mesh){o.geometry.dispose();(o.material as THREE.Material).dispose()}});if(this.shown)this.hideResult()}
}
