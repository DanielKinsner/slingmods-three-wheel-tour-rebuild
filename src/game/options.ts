...
import {DRIVER_SKILLS} from './driver-skills';
import {pointsRecords} from './points';
import {DRIFT_ZONES,zoneBoard} from './drift-zones';
import {RIVALS,rivalRecords,heatLabel} from './rivalry';
import {difficulty,setDifficulty,DIFFICULTIES,DIFFICULTY_LABEL,type Difficulty} from './difficulty';
import {MEDAL_NAMES,MEDAL_TARGETS,CREW_GHOST_TIMES,trialTime} from './time-attack';
import {CHALLENGES,KIND_LABEL,challengeRecords,challengeBest,formatValue} from './challenges';
...
function recordRows(){const names:Record<string,string>={harbor:'Original Harbor',express:'Harbor Express',ridge:'Smoky Ridge'},ride:Record<string,string>={'slingshot-r-2024':'Slingshot R','can-am-ryker-900':'Ryker 900'};
 return `<table class="gx-opt-controls gx-log-records"><thead><tr><th>Course</th><th>Ride</th><th>Best</th><th>Medal</th><th>Runs</th></tr></thead><tbody>${recordsTable().map(({route,rows})=>rows.map((r,i)=>`<tr>${i===0?`<td rowspan="${rows.length}"><b>${names[route]}</b><small>Gold ${trialTime(MEDAL_TARGETS[route].gold)} · Jett ${trialTime(CREW_GHOST_TIMES[route])}</small></td>`:''}<td>${ride[r.vehicle]??comparisonLabel(r.vehicle)}</td><td>${r.best?trialTime(r.best.timeMs):'—'}</td><td>${r.best?.medal?`<span class="gx-medal-chip" data-medal="${r.best.medal}">${MEDAL_NAMES[r.best.medal]}</span>`:'—'}</td><td>${r.best?.runs??0}</td></tr>`).join('')).join('')}</tbody></table><p class="gx-opt-note">Time Attack lives on the Race screen. Beat a medal time, then beat your own ghost.</p>${challengeRows()}${spyderChallengeRows()}`}
function challengeRows(){const u=speedUnits();return `<table class="gx-opt-controls gx-log-records"><thead><tr><th>Challenge</th><th>Type</th><th>Best</th><th>Medal</th><th>Runs</th></tr></thead><tbody>${CHALLENGES.map(c=>{const b=challengeBest(c.id);return `<tr><td><b>${c.title}</b><small>Gold ${formatValue(c,c.targets.gold,u)}</small></td><td>${KIND_LABEL[c.kind]}</td><td>${b?formatValue(c,b.value,u):'—'}</td><td>${b?.medal?`<span class="gx-medal-chip" data-medal="${b.medal}">${MEDAL_NAMES[b.medal]}</span>`:'—'}</td><td>${b?.runs??0}</td></tr>`}).join('')}</tbody></table><p class="gx-opt-note">Challenges live on the Race screen under each course.</p>`}
/** Driver skills unlocked by Tour Rep level (src/game/driver-skills.ts); used in Quick Race and test drives. */
function skillList(level:number){return `<section class="gx-skills" aria-label="Driver skills"><h3>Driver skills <small>Quick Race &amp; test drives</small></h3>${DRIVER_SKILLS.map(s=>`<div class="gx-skill-row" data-unlocked="${level>=s.level}"><b>LV ${s.level}</b><span><strong>${s.name}</strong>${s.detail}</span><em>${level>=s.level?'Unlocked':'Locked'}</em></div>`).join('')}</section>`}
/** Rivalries with the crew (game/rivalry.ts): head-to-head and heat. */
function rivalList(){const all=rivalRecords(),names:Record<string,string>={maya:'Maya',jett:'Jett',nico:'Nico'};return `<section class="gx-skills gx-rivals" aria-label="Rivalries"><h3>Rivalries <small>head-to-head across your races</small></h3>${RIVALS.map(id=>{const r=all[id];return `<div class="gx-skill-row" data-unlocked="${r.races>0}"><b>${r.ahead}\u2013${r.behind}</b><span><strong>${names[id]}</strong>${r.races?`${r.races} races \u00b7 ${heatLabel(r.heat)}`:'Not raced yet'}<i class="gx-heat" style="--heat:${r.heat}"></i></span><em>${r.races?(r.ahead>r.behind?'You lead':r.ahead<r.behind?'Trailing':'Level'):'\u2014'}</em></div>`}).join('')}</section>`}
/** Drift zone bests (local top-five boards, game/drift-zones.ts). */
function zoneList(){const routes:Record<string,string>={harbor:'Original Harbor',express:'Harbor Express',ridge:'Smoky Ridge'};return `<section class="gx-skills gx-zones" aria-label="Drift zone bests"><h3>Drift zones <small>best score per zone</small></h3>${DRIFT_ZONES.map(z=>{const b=zoneBoard(z.id);return `<div class="gx-skill-row" data-unlocked="${b.length>0}"><b>${b[0]?b[0].score.toLocaleString('en-US'):'\u2014'}</b><span><strong>${z.name}</strong>${routes[z.route]}${b.length>1?` \u00b7 ${b.length} runs on the board`:''}</span><em>${b.length?'Top 5':'Open'}</em></div>`}).join('')}</section>`}
function statRows(){const c=careerSource(),p=tourProgress(c),s=driveStats();const tile=(v:string,l:string)=>`<div class="gx-stat"><b>${v}</b><span>${l}</span></div>`;
 return `<div class="gx-stats">${tile('LV '+p.level,p.title)}${tile(p.rep.toLocaleString('en-US'),'Tour Rep')}${tile(c?p.credits.toLocaleString('en-US')+' CR':'—','Credits')}${tile(p.completed+'/'+p.total,'Career events')}${tile(String(s.drives),'Drives started')}${tile(String(s.races),'Races')}${tile(String(s.trials),'Time Attacks')}${tile(String(s.tests),'Test drives')}${tile(String(s.perfectStarts),'Perfect starts')}${tile(String(s.cleanWins),'Clean wins')}${tile(skillRecord().best.toLocaleString('en-US'),'Best skill chain')}${tile(skillRecord().total.toLocaleString('en-US'),'Skill points')}${tile(Object.values(pointsRecords()).reduce((a,r)=>a+(r.lifetime||0),0).toLocaleString('en-US'),'SlingMods Points')}</div>${skillList(p.level)}${rivalList()}${zoneList()}${c?'':'<p class="gx-opt-note">Open the career from the main menu to include its progress here.</p>'}${c?.ownBuild.ridge.completed['summit-invitational']?'<button class="gx-opt-cta" data-opt="finale">Watch the Tour finale</button>':''}`}
function render(){
...
function onClick(e:Event){const t=(e.target as Element).closest<HTMLElement>('[data-opt],[data-gx-tab]');if(!t)return;
 ...
 if(o==='rumble'){setRumbleEnabled(!rumbleEnabled());if(rumbleEnabled())pulse(.6,.6,200);render();focusOpt('rumble')}
...
function spyderChallengeRows(){return `<h3>Spyder challenge builds</h3><table class="gx-opt-controls gx-log-records"><tbody>${CHALLENGES.flatMap(c=>challengeRecords(c.id).filter(r=>r.key.includes('can-am-spyder-f3')).map(r=>`<tr><td>${c.title}</td><td>${comparisonLabel(r.best.vehicle)}</td><td>${formatValue(c,r.best.value,speedUnits())}</td><td>${r.best.medal??'—'}</td></tr>`)).join('')}</tbody></table>`}
