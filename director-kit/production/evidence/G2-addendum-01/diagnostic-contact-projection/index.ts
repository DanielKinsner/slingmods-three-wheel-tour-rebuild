...
export class Simulation {
  ...
  step(control:VehicleControl,dt=FIXED_DT):void {
    ...
    // Couple the engine to the same rear contact-point longitudinal projection used below.
    // Overspeed is sampled at tick start; tire force updates it later in this tick.
    const rearSample=samples[2];
    const rearForward=rearSample.hit?normalized(add(forward,scale(rearSample.hit.normal,-dot(forward,rearSample.hit.normal)))):forward;
    const rearLongitudinal=rearSample.contact?dot(rearSample.pointVel,rearForward):speed;
    const driveState=this.drivetrain.step(speed,this.overspeed[2],layout.wheels[2].radius,this.throttle,this.brake,Boolean(control.reverse),dt,rearLongitudinal);
