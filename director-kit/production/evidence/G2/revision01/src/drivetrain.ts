...
export class AutoDrive {
 gear=1;rpm:number=DRIVETRAIN.idle;shiftRemaining=0;reversePending=false;private shiftTarget=1;
 reset():void{this.gear=1;this.rpm=DRIVETRAIN.idle;this.shiftRemaining=0;this.shiftTarget=1;this.reversePending=false}
 step(speed:number,rearOverspeed:number,radius:number,requestedThrottle:number,requestedBrake:number,reverse:boolean,dt:number){
  ...
  if(this.reversePending&&Math.abs(speed)<0.5){this.gear=reverse?-1:1;this.shiftRemaining=DRIVETRAIN.shiftSeconds;this.shiftTarget=this.gear;this.reversePending=false}
  ...
  const wheelRpm=(Math.abs(speed)/radius+Math.abs(rearOverspeed))*60/(2*Math.PI);
  ...
  if(this.shiftRemaining===0&&this.gear>0){if(this.rpm>6900&&this.gear<5){this.shiftTarget=this.gear+1;this.shiftRemaining=DRIVETRAIN.shiftSeconds}else if(this.rpm<2500&&this.gear>1){this.shiftTarget=this.gear-1;this.shiftRemaining=DRIVETRAIN.shiftSeconds}}
  const force=this.shiftRemaining>0||this.rpm>8400?0:engineTorque(this.rpm)*throttle*ratio*DRIVETRAIN.finalDrive*DRIVETRAIN.efficiency/radius*(this.gear<0?-1:1)*(this.gear<0?clamp((6+speed)/2,0,1):1);
