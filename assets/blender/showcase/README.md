...
The 2048×2048 original sixteen-tile atlas has sRGB base color and separate linear ORM and +Y tangent normal images. Materials distinguish concrete render, coated metal, timber, leaf, stone, restrained red, inset glass, cladding, hull, rubber, service yellow and epoxy. The garage floor is a single planar quad with a dedicated 512-pixel seamlessly repeating epoxy basecolor/ORM/normal set, derived from the original authored epoxy tile and repeated at approximately 0.6 metres. Ordinary mipmaps remain enabled; separate maps prevent cross-tile atlas mip bleed. Water has its own 512-pixel periodic tangent normal at a 12-metre UV repeat, animated by texture offset; it uses the existing environment reflection, with no extra scene reflection pass.
