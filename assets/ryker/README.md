...
The existing free drives, races and career transitions use the selected appearance. Slingshot catalog geometry is suppressed on the Ryker. The original sage panels/tan seat are its default finish; the paint panels remain adjustable. The purchased instrument housing now carries an original SIM readout using actual game speed, RPM and gear. It goes dark with ignition; no OEM artwork or unsupported sensor readings are claimed.

## Files and reproducibility

- `public/assets/ryker/ryker-900.glb`: self-contained gameplay model, **719,254 triangles, 13,469,976 bytes**, 20 PBR materials, no image dependencies or compression decoder.
