...
The existing free drives, races and career transitions use the selected appearance. Slingshot catalog geometry is suppressed on the Ryker. The original sage panels/tan seat are its default finish; the paint panels remain adjustable. The source instrument stays blank and addressable; no invented OEM instrument graphics were added.

## Files and reproducibility

- `public/assets/ryker/ryker-900.glb`: self-contained gameplay model, **718,634 triangles, 13,453,744 bytes**, 19 PBR materials, no image dependencies or compression decoder.
...
The rider repair reduced measured grip gaps from about 16 cm to below 0.001 mm in the captured driven pose. This proves bone contact targets, not human ergonomic certification. Existing cockpit camera eye placement is adapted to the rider; generic showroom presets remain shared. Mirrors use their supplied reflective materials; a Ryker live-mirror implementation was not added.
